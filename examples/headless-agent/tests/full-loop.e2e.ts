...
describe.skipIf(!process.env.HIVEFORGE_API_KEY)('full loop: real model + real bash tool', () => {
  it('runs a bash command on request and reports its output', async () => {
    ...
    const agent = ctx.agentLoop.create(SessionId('e2e-loop'), { provider: 'hiveforge-official', model: 'hiveforge-v4-flash' })
