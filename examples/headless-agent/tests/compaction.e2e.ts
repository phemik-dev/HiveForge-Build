...
describe.skipIf(!process.env.HIVEFORGE_API_KEY)('compaction: a long session compacts mid-flight and keeps running', () => {
  it('summarizes older history into a checkpoint without breaking the task', async () => {
    ...
    const agent = ctx.agentLoop.create(SessionId('e2e-compaction'), { provider: 'hiveforge-official', model: 'hiveforge-v4-flash' })
