# HiveForge Harness Python SDK

English | [中文](https://github.com/hiveforge-ai/hiveforge-harness/blob/master/python/sdk/README.zh.md)

Python subprocess SDK for driving HiveForge Harness over JSON-RPC stdio. The
runtime inherits normal HiveForge Harness environment variables such as
`HIVEFORGE_BASE_URL` and `HIVEFORGE_API_KEY`, so callers can use real model
endpoints directly or point those variables at a local proxy.

Install the `hiveforge-harness-sdk` distribution from PyPI; the import module remains `hiveforge_harness`:

```sh
python -m pip install hiveforge-harness-sdk
```

Installing `hiveforge-harness-sdk` installs the exact same-version `hiveforge-harness-runtime-bin` platform wheel. The normal entry point therefore needs no executable argument:
...
`HiveForgeHarness` keeps its lazily started runtime subprocess for reuse across calls. Use it as a context manager, as above, or call `close()` explicitly when finished.

By default, the SDK launches the bundled single-file `dsh-jsonrpc-agent` executable from the `hiveforge-harness-runtime-bin` package and injects that package's default configuration (the stdio JSON-RPC server, agent core, preloaded HiveForge adapter, JSONL session persistence with an explicitly composed semantic checkpoint policy, local bash) via `DSH_CORDIS_CONFIG`. To run a plugin composition of your own, keep the `@hiveforge-ai/dsh-sdk-jsonrpc-server` entry in the config and pass the Cordis config path.
...
The same behavior can be selected for the runtime subprocess with `DSH_CORDIS_CONFIG`. The injection lives in `HarnessClient.start()`, so the low-level client's default launch gets it too: when the launch resolves to the bundled runtime and neither `cordis` nor a non-empty `DSH_CORDIS_CONFIG` is set (the runtime treats an empty value as absent, and so does the injection check), the bundled default configuration is used; an explicit `runtime_bin`, `bridge_bin`, or `launch_args_override` disables the injection entirely. See the [sdk-runtime README](https://github.com/hiveforge-ai/hiveforge-harness/blob/master/python/sdk-runtime/README.md) for the runtime carriers (production exe vs dev-only node closure) and how to obtain them.
