...
class HiveForgeHarnessConfig:
    ...
    HIVEFORGE_API_KEY and HIVEFORGE_BASE_URL settings keep working. Use ``env`` to
    intentionally override or inject variables for a subprocess.
    """

    provider: str = "hiveforge-official"
    model: str = "hiveforge-v4-flash"
...
class HiveForgeHarness:
    """Reusable synchronous SDK for running HiveForge Harness agent turns.
    ...
    def __init__(self, config: HiveForgeHarnessConfig | None = None, **kwargs: object) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either HiveForgeHarnessConfig or keyword options, not both")
        self.config = config or HiveForgeHarnessConfig(**kwargs)
        ...
        if self.config.base_url is not None:
            env["HIVEFORGE_BASE_URL"] = self.config.base_url
        if self.config.api_key is not None:
            env["HIVEFORGE_API_KEY"] = self.config.api_key
    ...
    def __enter__(self) -> "HiveForgeHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: HiveForgeHarness, session_id: str) -> None:
