...
class HarnessConfig:
    """Configuration for launching the local HiveForge Harness SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the HiveForge Harness SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        try:
            ...
            proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("HiveForge Harness runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for HiveForge Harness runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("HiveForge Harness runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to HiveForge Harness runtime") from exc
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("HiveForge Harness runtime stdout closed"))
    ...
    def _default_launch_args(self) -> tuple[str, ...]:
        ...
        try:
            from hiveforge_harness_runtime import resolve_bundled_launch_args
        except ImportError as exc:
            raise FileNotFoundError(
                "Unable to locate the bundled HiveForge Harness SDK runtime. "
                "Install hiveforge-harness-runtime-bin or set HarnessConfig.runtime_bin."
            ) from exc
    ...
    def _inject_bundled_default_config(self, env: dict[str, str]) -> None:
        ...
        from hiveforge_harness_runtime import bundled_default_config_path
