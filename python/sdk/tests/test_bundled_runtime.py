...
from hiveforge_harness import HiveForgeHarness, HarnessClient, HarnessConfig
from hiveforge_harness.errors import TransportClosedError
...
- id: sdk-jsonrpc-server
  name: '@hiveforge-ai/dsh-sdk-jsonrpc-server'
- id: agent-core
  name: '@hiveforge-ai/dsh-agent-spine-demo'
  config:
...
- id: session-checkpoints
  name: '@hiveforge-ai/dsh-session-checkpoint-policy'
- id: subprocess
    ...
    return HarnessClient(
        HarnessConfig(
            ...
            env={
                ...
                "HIVEFORGE_API_KEY": "sk-dummy-for-boot",
                "HIVEFORGE_BASE_URL": "http://127.0.0.1:9",
            },
...
def test_bundled_runtime_boots_a_cordis_config(tmp_path: Path, mode: str) -> None:
    ...
    with _client(tmp_path, launch_args) as client:
        init = client.initialize(provider="hiveforge-official", cwd=str(tmp_path), model="hiveforge-v4-pro")

    assert init.serverInfo is not None
    assert init.serverInfo.name == "hiveforge-harness-sdk-runtime"
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    (tmp_path / "cordis.yml").write_text(
        "- id: missing\n  name: '@hiveforge-ai/dsh-does-not-exist'\n"
    )
    ...
    try:
        with pytest.raises((TransportClosedError, TimeoutError)) as excinfo:
            client.initialize(provider="hiveforge-official", cwd=str(tmp_path), model="hiveforge-v4-pro")
    finally:
        client.close()

    assert "@hiveforge-ai/dsh-does-not-exist" in str(excinfo.value)
