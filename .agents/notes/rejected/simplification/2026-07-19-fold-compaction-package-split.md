...
Compaction is split between `@hiveforge-ai/dsh-compaction`, which owns an abstract two-method service and shared types, and `@hiveforge-ai/dsh-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@hiveforge-ai/dsh-compaction` and remove `@hiveforge-ai/dsh-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@hiveforge-ai/dsh-compaction-basic` and its workspace/package metadata are removed.
- `@hiveforge-ai/dsh-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
