...
`scripts/install.sh` 现有一套位于 `apps/cli/tests/install-script.spec.ts` 的真实 shell PTY 回归测试，使用 stub 依赖覆盖接管路径和 curl 风格路径。curl 风格安装默认使用公开的 `hiveforge-ai/hiveforge-harness-sdk` 源，而以 pnpm/npx 替换安装器仍是另一项工作。
