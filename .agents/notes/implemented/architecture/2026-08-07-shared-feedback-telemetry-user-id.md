# Agent Note: Shared anonymous user id across telemetry, feedback, and HiveForge requests
...
The earlier [anonymous-user-id decision](../feature/2026-07-31-telemetry-anonymous-user-id.md) deliberately kept the helper inside the OTel backend until a second real consumer existed. Feedback became that second consumer. [Direct HiveForge request identity](../feature/2026-08-11-hiveforge-request-user-id-header.md) is the third.

## Decision

`@hiveforge-ai/dsh-anonymous-user-id` owns `getOrCreateAnonymousUserId()` and the `$DSH_HOME/.anonymous-user-id` storage contract. `session-telemetry-otel` uses the returned id as OpenTelemetry Resource `user.id`; the `/feedback` success acknowledgement reports `Feedback recorded for session {sessionId}` followed by `Anonymous user: {userId}` on a second line; and direct HiveForge requests carry it as `x-hiveforge-harness-user-id`. Invalid feedback is rejected before resolving the id, and the HiveForge adapter resolves it only after credentials succeed, so neither an empty command nor a credential failure creates `.anonymous-user-id`.
...
- One harness home has one anonymous id shared by feedback acknowledgements, session telemetry exports, and direct HiveForge requests.
