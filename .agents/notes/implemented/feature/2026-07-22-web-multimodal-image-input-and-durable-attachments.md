...
| `packages/llm/llm-hiveforge` | Resolve official vision input to deterministic request versions and Files API ids. |
...
- Adapter and compaction tests cover deterministic Pi-AI request versions, HiveForge Files upload and reuse, stale-id recovery, text-only projection, recursively nested tool-result images, shared summary request versions, and explicit image-output rejection.
...
- Credentialed real-API tests cover the configured Anthropic route and the built-in `hiveforge-official` Files path. The HiveForge test does not use a custom provider entry.
