...
同权政策已由[英文默认的产品与文档文案](2026-08-27-english-default-product-copy.zh.md)部分取代；配对文件、伴随记录与一致性门禁决策仍然有效。
...
- **全语料统一要求。** 范围内的每篇文档从创建起就必须有完整配对；政策没有逐文件推进状态、日期分界或 README 专用类别。README 发现会覆盖 vendor 源码、依赖目录与被忽略的构建产物目录之外所有文件名不区分大小写匹配 README 的文件，包括今后新增的顶层目录。发布到文档站的配对使用 `pairedPages()`，由根 locale 投影 `.md`，由 `/zh/` 投影 `.zh.md`；仅创建对侧文件并不会发布它。
