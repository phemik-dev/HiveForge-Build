/**
 * Named wire types for the HiveForge Harness SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@hiveforge-ai/dsh-sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `hiveforge-harness-sdk-runtime`.
 *
 * @module @hiveforge-ai/dsh-sdk-protocol/types
 */

import type { ContentBlock } from '@hiveforge-ai/dsh-llm'
import type { SessionEvent } from '@hiveforge-ai/dsh-session'
import type { SubagentStopReason } from '@hiveforge-ai/dsh-subagent'
...
export interface InitializeResult {
  /** Wire-stable server identity (`hiveforge-harness-sdk-runtime`) and version. */
