...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): HiveForgeHarness {
  const harness = new HiveForgeHarness({ launch: fakeLaunch(env, extra) })
...
describe('HiveForgeHarness', () => {
  it('ignores notifications that precede the submitted message receipt', async () => {
    ...
    } as unknown as HiveForgeHarness
  ...
  it('supports await using disposal', async () => {
    let captured: HiveForgeHarness
    {
      await using harness = new HiveForgeHarness({ launch: fakeLaunch() })
      captured = harness
...
describe('HarnessClient', () => {
  ...
  it('delivers notifications to unfiltered and filtered subscriptions in wire order', async () => {
    ...
    expect(identity.serverInfo.name).toBe('hiveforge-harness-sdk-runtime')
