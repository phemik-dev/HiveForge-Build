/**
 * Low-level JSON-RPC client for a HiveForge Harness SDK runtime subprocess.
 * {@link HarnessClient} owns the child process: it spawns the runtime, speaks
 * the `@hiveforge-ai/dsh-sdk-protocol` wire over the child's stdio, fans
 ...
 *
 * @module @hiveforge-ai/dsh-sdk-client/client
 */
...
} from '@hiveforge-ai/dsh-sdk-protocol'
...
/**
 * JSON-RPC client for the HiveForge Harness SDK runtime over subprocess stdio.
 *
...
export class HarnessClient {
   ...
   */
  start(): void {
    if (this.closeTask !== undefined) throw new TransportClosedError('HiveForge Harness runtime client is closed')
    ...
    child.once('error', (error) => {
      ...
      this.failSubscriptions(this.closedError('HiveForge Harness runtime failed to start'))
    })
    ...
    child.once('exit', (code) => {
      ...
      settled.exited = true
      maybeSettle()
      this.failSubscriptions(this.closedError('HiveForge Harness runtime exited'))
    })
    child.once('close', () => {
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    ...
    if (this.exitCode !== undefined || this.spawnError !== undefined) {
      ...
      throw this.closedError('HiveForge Harness runtime is not running')
    }
    const transport = this.transport
    /* v8 ignore next -- start() either sets the transport or throws */
    if (transport === undefined) throw new TransportClosedError('HiveForge Harness runtime is not running')
    ...
    try {
      ...
      const timer = setTimeout(() => {
        abandon.abort(new RequestTimeoutError(`${method} timed out after ${timeout}ms waiting for the HiveForge Harness runtime`))
      }, timeout)
      try {
  ...
  subscribe(filter?: NotificationFilter): NotificationSubscription {
    ...
    if (this.closeTask !== undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      subscription.fail(this.closedError('HiveForge Harness runtime closed'))
      return subscription
    }
  ...
  private async performClose(): Promise<void> {
    ...
    this.transport?.close()
    this.failSubscriptions(this.closedError('HiveForge Harness runtime closed'))
  }
