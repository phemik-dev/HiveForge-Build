/**
 * High-level run API over {@link HarnessClient}: `HiveForgeHarness` owns one
 ...
 * Mirrors the Python SDK's `HiveForgeHarness`/`Session` pair.
 *
 * @module @hiveforge-ai/dsh-sdk-client/api
 */
...
import type { ContentBlock, HiveForgeHarnessOptions, HarnessClientOptions, HarnessNotification, RunResult } from './types.ts'

/**
 * Reusable SDK for running HiveForge Harness agent turns in a runtime
 ...
 * child is reaped.
 */
export class HiveForgeHarness implements AsyncDisposable {
  ...
  constructor(options: HiveForgeHarnessOptions) {
    ...
    this.provider = options.provider ?? 'hiveforge-official'
    this.model = options.model ?? 'hiveforge-v4-flash'
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: HiveForgeHarness, readonly id: string) {}
