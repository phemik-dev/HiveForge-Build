 ...
 *
 * @module @hiveforge-ai/dsh-sdk-jsonrpc-server/server
 */

import type { Context } from '@hiveforge-ai/cordis'
import { resolve } from 'node:path'
import type { Agent, AgentHandle } from '@hiveforge-ai/dsh-agent'
import { createUserMessage } from '@hiveforge-ai/dsh-llm'
import { carrierKeyOf, type Scoped } from '@hiveforge-ai/dsh-scope'
import { SessionId } from '@hiveforge-ai/dsh-session'
import type SubagentRuntime from '@hiveforge-ai/dsh-subagent'
import type { SubagentRunEndInfo } from '@hiveforge-ai/dsh-subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  private provider = 'hiveforge-official'
  private model = 'hiveforge-official'
  ...
  /**
   * Configure the SDK route, mounting the HiveForge fallback only when unowned.
  ...
  async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    if (!this.hasAdapterFor(this.provider)) {
      if (this.provider !== 'hiveforge-official') throw new Error(`no adapter registered for provider "${this.provider}"`)
      this.llmFiber = await this.ctx.plugin(LlmHiveForge, {})
    }
    return { serverInfo: { name: 'hiveforge-harness-sdk-runtime', version: '0.0.1' } }
  }
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown HiveForge Harness SDK runtime method: ${method}`)
    }
  }
  ...
  private async createSession(sessionId: string): Promise<SessionRecord> {
    ...
    // (@hiveforge-ai/dsh-agent-presets README, "Composing a child agent").
