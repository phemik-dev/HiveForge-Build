  ...
  it('scrubs ambient credentials but forwards explicit config env', async () => {
    ...
    try {
      const ctx = await setup({
        FAKE_ECHO_ENV: 'DSH_TEST_AMBIENT_SECRET_KEY,HIVEFORGE_API_KEY',
        HIVEFORGE_API_KEY: 'explicit-child-key',
        FAKE_TEXT: 'done',
      })
      const run = await ctx.subagents.start('dsh-sdk', request())
      const result = await run.result
      ...
      expect(answer).toContain('HIVEFORGE_API_KEY=explicit-child-key')
      await run.dispose()
      await ctx.fiber.dispose()
    } finally {
