...
function hiveForgeBaseUrl(): string {
  ...
  if (configured !== OFFICIAL_HIVEFORGE_BASE_URL) {
    throw new Error('Claude Code HiveForge e2e requires the official HiveForge base URL')
  }
  return configured
}
...
describe.skipIf(!process.env.HIVEFORGE_API_KEY)(
  'Claude Code provider with real HiveForge API',
  () => {
    it('returns one unique nonce through the production provider and real SDK/CLI', async () => {
      ...
      const root = mkdtempSync(join(tmpdir(), 'dsh-claude-hiveforge-e2e-'))
      roots.push(root)
      ...
      const env = {
        ...
        ANTHROPIC_BASE_URL: `${hiveForgeBaseUrl()}/anthropic`,
        ANTHROPIC_MODEL: 'hiveforge-v4-pro[1m]',
        ANTHROPIC_DEFAULT_OPUS_MODEL: 'hiveforge-v4-pro[1m]',
        ANTHROPIC_DEFAULT_SONNET_MODEL: 'hiveforge-v4-pro[1m]',
        ANTHROPIC_DEFAULT_HAIKU_MODEL: 'hiveforge-v4-flash',
        CLAUDE_CODE_SUBAGENT_MODEL: 'hiveforge-v4-flash',
      ...
      const nonce = `DSH_CLAUDE_HIVEFORGE_${randomUUID()}`
      const parent = {
