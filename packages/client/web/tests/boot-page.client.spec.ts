...
describe('BootPage', () => {
  it('draws the loading skeleton before any plugin state arrives', () => {
    ...
    expect(el.textContent).toContain('Forge Ahead')
    expect(el.textContent).toContain('Provisional identity · Loading plugins…')
  })
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@hiveforge-ai/dsh-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@hiveforge-ai/dsh-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@hiveforge-ai/dsh-client-ui-layout')
    expect(el.textContent).toContain('@hiveforge-ai/dsh-client-ui-tool')
    expect(el.textContent).not.toContain('ok')
    expect(el.textContent).not.toContain('Provisional identity · Loading plugins…')
  })
