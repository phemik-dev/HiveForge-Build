...
describe('createFixtureApi', () => {
  ...
  it('serves configured HiveForge readiness and keeps credential values write-only', async () => {
    const api = createFixtureApi()
    ...
    const initial = await api.credentials.describe(req({ refs: ['HIVEFORGE_API_KEY', 'TEST_API_KEY'] }))
    if (!initial.result.ok) throw new Error('credential describe failed')
    expect(initial.result.value.credentials).toEqual({
      HIVEFORGE_API_KEY: { configured: true, source: 'file', writable: true },
