 ...
 * not built (`pnpm --filter @hiveforge-ai/dsh-client-ui-trajectory bundle`).
 */
...
const PLUGIN_ID = '@hiveforge-ai/dsh-client-ui-trajectory'
...
describe('tsdown client artifact', () => {
  ...
  function itBuilt(name: string, fn: () => Promise<void>): void {
    ...
    it(name, { timeout: process.platform === 'win32' ? 60_000 : 20_000 }, fn)
  }
  ...
  async function loadArtifact() {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@hiveforge-ai/dsh-client-runtime/client', await import('@hiveforge-ai/dsh-client-runtime/client')],
      ['@hiveforge-ai/dsh-client-ui-primitives', await import('@hiveforge-ai/dsh-client-ui-primitives')],
    ])
  ...
  itBuilt('mounted as an object plugin, apply registers the view tab on the real ring', async () => {
    ...
    const locale = await import('@hiveforge-ai/dsh-client-locale/client')
