...
宿主端在用户设置 seam 中注册 `ui-onboarding`。功能引导步骤会把持久化标记写在这里：`ui-settings-models` 提供的欢迎步骤通过既有公开 settings 边界读写 `welcomeNoticeVersion`，旧配置导入步骤则以 best-effort 写入 `legacyImportDismissed` / `legacyImportCompleted`。外壳本身仍不持有产品策略。
