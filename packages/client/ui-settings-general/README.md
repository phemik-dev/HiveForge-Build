# @hiveforge-ai/dsh-client-ui-settings-general
...
The Host half registers `ui-onboarding` in the user-settings seam. Feature onboarding steps store durable flags there: the welcome step contributed by `ui-settings-models` reads and writes `welcomeNoticeVersion`, and the legacy-import step records `legacyImportDismissed` / `legacyImportCompleted` best-effort. The shell itself remains policy-free.
