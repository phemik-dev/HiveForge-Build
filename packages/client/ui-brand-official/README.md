...
This package fills `sidebar.brand.mark`, `sidebar.brand.name`, and `conversation.hero.brand.mark` only when `DSH_CLIENT_BUILD_PROFILE` is `official`. The official sidebar identity renders the approved mark beside `HiveForge` and an explicit `Provisional` status label. Other builds load the plugin but register no occupants, leaving the shell fallbacks visible.
