...
仅当 `DSH_CLIENT_BUILD_PROFILE` 为 `official` 时，本包才填充 `sidebar.brand.mark`、`sidebar.brand.name` 和 `conversation.hero.brand.mark`。官方侧边栏身份在批准的标记旁显示 `HiveForge`，并附带明确的 `Provisional` 状态标签。其他构建仍会加载插件，但不注册 occupant，因此显示 shell fallback。
