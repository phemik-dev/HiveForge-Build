...
import type {} from '@hiveforge-ai/dsh-plan-mode/client'
// Type-only: the `goal` projection key merge (hint disambiguation).
import type {} from '@hiveforge-ai/dsh-goal/client'
...
import type { Translate } from '@hiveforge-ai/dsh-client-ui-slots'
...
export function InputBar({
  ...
  // Intake pre-check (HiveForge Chat semantics): an addition that would break
  ...
  const intakeImages = useCallback((files: readonly File[]): void => {
    ...
    const rejected = ((): string | null => {
      if (imageLimits !== undefined) {
        // Format precedes limits (HiveForge Chat's filter order): a batch with
