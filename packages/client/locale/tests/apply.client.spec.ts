...
} from '@hiveforge-ai/dsh-client-locale/client'
import type { LanguageRowInjected, LocaleRuntime } from '@hiveforge-ai/dsh-client-locale/client'
...
describe('locale apply', () => {
  ...
  it('projects service snapshots into the row store and routes face writes back', async () => {
    ...
    expect(instance.getSnapshot().options.map(o => o.id)).toEqual(['en', 'zh'])
