...
import { createScope, scopeOf, SlotRegistry } from '@hiveforge-ai/dsh-client-runtime/client'
import type { SessionId } from '@hiveforge-ai/dsh-client-runtime/client'
import { apply, inject, InputTriggerService } from '@hiveforge-ai/dsh-client-ui-input-trigger/client'
import type { MenuViewInjected } from '@hiveforge-ai/dsh-client-ui-input-trigger/client'
...
describe('apply', () => {
  ...
  it('registers the bilingual menu dictionaries (group titles by source name + the pending row)', async () => {
    ...
    expect(t('subagent')).toBe('Supporting agents')
