...
import Schema from '@hiveforge-ai/schemastery'
import { bindSnapshotSelector } from '@hiveforge-ai/dsh-client-test-runtime'
import type { RpcResponse, SettingsNamespaceView } from '@hiveforge-ai/dsh-api-remotes/client'
import {
...
import {
  HiveForgeModelsEditor, formatCapacity, modelDrafts, parseCapacity, validateHiveForgeModels,
} from '../src/client/HiveForgeModelsEditor.tsx'
import { apiKeyFailure } from '../src/client/apiKey.ts'
import { SettingsDescribeMirror } from '@hiveforge-ai/dsh-client-ui-settings/src/client/settings-mirror.ts'
...
const HIVEFORGE_TARGET = { provider: 'hiveforge-official', displayName: 'HiveForge' }
const hiveForgeCopy = (template: string): string => providerCopy(template, HIVEFORGE_TARGET)
...
const HiveForgeConfig = Schema.object({
  apiKeyEnv: Schema.string().role('credential-ref'),
...
const DEFAULT_HIVEFORGE_MODELS = [
  {
    id: 'hiveforge-v4-flash',
    name: 'V4 Flash',
  ...
  { id: 'hiveforge-v4-pro', name: 'V4 Pro', contextWindow: 1_000_000 },
]
...
function wireNamespaces(): SettingsNamespaceView[] {
  return [
    {
      ns: 'llm-hiveforge',
      schema: JSON.parse(JSON.stringify(HiveForgeConfig.toJSON())) as unknown,
      value: {
        apiKeyEnv: 'HIVEFORGE_API_KEY',
        baseURL: 'https://base',
        defaultContextWindow: 1_000_000,
        maxTokens: 256_000,
        models: DEFAULT_HIVEFORGE_MODELS,
      },
      base: { defaultContextWindow: 1_000_000, maxTokens: 256_000, models: DEFAULT_HIVEFORGE_MODELS },
 ...
 * anywhere, so the whole-section HiveForge route owns the first-run setup card.
 */
...
/**
 * Mount and open the HiveForge editor. The shared fixture already has a usable
 * openai route, so HiveForge is an ordinary row whose card opens through Edit
 * rather than by itself.
 */
async function mountHiveForgeCard(overrides: Parameters<typeof scriptedFace>[0] = {}) {
  const mounted = await mountSection(overrides)
  fireEvent.click(screen.getByRole('button', { name: hiveForgeCopy(en.editProvider) }))
  return mounted
}
...
describe('ModelsSection', () => {
  ...
  it('renders the unkeyed whole-section provider as an open setup card in the first-run posture', async () => {
    await mountFirstRun()
    // Nothing is reachable yet, and HiveForge has no configured credential and
  ...
  it('decides setup need from the joined credential state and the first-run posture', () => {
    const entry = { provider: 'p', displayName: 'p', settingsNs: 'llm-hiveforge', settingsPath: [], active: true }
  ...
  it('uses one stable provider identity in action copy', () => {
    const target = { provider: 'hiveforge-official', displayName: 'HiveForge' }
  ...
  it('stores a typed key write-only from the setup card without touching settings', async () => {
    ...
    await waitFor(() => { expect(set).toHaveBeenCalledWith({ ref: 'HIVEFORGE_API_KEY', value: 'sk-live' }) })
    ...
    expect((await screen.findByRole('status')).textContent).toBe(
      providerCopy(en.savedProvider, { provider: 'hiveforge-official', displayName: 'HiveForge' }),
    )
  ...
  it('reuses the provider editor as a required credential-only onboarding form', async () => {
    ...
    expect(set).toHaveBeenCalledWith({ ref: 'HIVEFORGE_API_KEY', value: 'sk-onboarding' })
  ...
  it('applies customized hiveforge fields as path ops', async () => {
    const { mutate } = await mountHiveForgeCard({
      mutate: vi.fn(() => Promise.resolve(ok(wireNamespaces()[0]))),
    })
    fireEvent.click(screen.getByText(en.customized))
    const baseURL = screen.getByLabelText<HTMLInputElement>(en.baseUrl)
    // The hiveforge placeholder is pinned to the public endpoint, not the
  ...
  it('materializes inherited models and adds an arbitrary HiveForge id', async () => {
    const { mutate } = await mountHiveForgeCard({
      mutate: vi.fn(() => Promise.resolve(ok(wireNamespaces()[0]))),
    })
    fireEvent.click(screen.getByText(en.customized))
    ...
    expect(screen.getAllByLabelText(new RegExp(en.modelId)).map(input => (input as HTMLInputElement).value))
      .toEqual(['hiveforge-v4-flash', 'hiveforge-v4-pro'])
  ...
  it('rejects duplicate HiveForge model ids before writing', async () => {
    const { mutate } = await mountHiveForgeCard()
    fireEvent.click(screen.getByText(en.customized))
    fireEvent.click(screen.getByText(en.addModel))
    const ids = screen.getAllByLabelText(new RegExp(en.modelId))
    fireEvent.change(ids[2] as HTMLInputElement, { target: { value: 'hiveforge-v4-flash' } })
    fireEvent.click(screen.getByText(en.apply))
  ...
  it('validates every adapter-owned model catalog invariant', () => {
    ...
    expect(validateHiveForgeModels([{}])).toEqual({ index: 0, key: 'modelIdRequired' })
    expect(validateHiveForgeModels([{ id: 'same' }, { id: 'same' }]))
      .toEqual({ index: 1, key: 'modelIdDuplicate' })
    expect(validateHiveForgeModels([{ id: 'model', name: '' }]))
      .toEqual({ index: 0, key: 'modelNameInvalid' })
    expect(validateHiveForgeModels([{ id: 'model', contextWindow: null }]))
      .toEqual({ index: 0, key: 'modelContextInvalid' })
    expect(validateHiveForgeModels([{ id: 'model', contextWindow: 1.5 }]))
      .toEqual({ index: 0, key: 'modelContextInvalid' })
    expect(validateHiveForgeModels([{ id: 'model', contextWindow: 0 }]))
      .toEqual({ index: 0, key: 'modelContextInvalid' })
    expect(validateHiveForgeModels([{ id: 'model', contextWindow: 1 }])).toBeUndefined()
    expect(validateHiveForgeModels([{ id: 'model', maxTokens: null }]))
      .toEqual({ index: 0, key: 'modelMaxTokensInvalid' })
    expect(validateHiveForgeModels([{ id: 'model', maxTokens: 1.5 }]))
      .toEqual({ index: 0, key: 'modelMaxTokensInvalid' })
    expect(validateHiveForgeModels([{ id: 'model', maxTokens: 0 }]))
      .toEqual({ index: 0, key: 'modelMaxTokensInvalid' })
    expect(validateHiveForgeModels([{ id: 'model', maxTokens: 8192 }])).toBeUndefined()
  })
  ...
  it('accepts a suffixed context window and stores the plain count', async () => {
    ...
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ...
      ops: [{
        ...
        path: ['models'],
        value: [
          { ...DEFAULT_HIVEFORGE_MODELS[0], contextWindow: 1_000_000 },
          { ...DEFAULT_HIVEFORGE_MODELS[1], contextWindow: 256_000 },
        ],
      }],
      expectedRevision: 0,
    })
  ...
  it.each([
    ...
    expect(screen.getByText(en.modelsInherited)).toBeTruthy()
    expect(screen.getAllByLabelText(new RegExp(en.modelId)).map(input => (input as HTMLInputElement).value))
      .toEqual(base === undefined ? ['hiveforge-v4-flash', 'hiveforge-v4-pro'] : ['pinned-by-deployment'])
  })
  ...
  it('edits an output cap per model and carries its text across a removal', async () => {
    ...
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ...
      ops: [{
        op: 'set',
        path: ['models'],
        value: [{ ...DEFAULT_HIVEFORGE_MODELS[1], maxTokens: 64_000 }],
      }],
      expectedRevision: 0,
    })
  })
  ...
  it('settles a pasted id and refuses whitespace that would never match', async () => {
    ...
    fireEvent.change(ids[0] as HTMLInputElement, { target: { value: '  hiveforge-v4-flash  ' } })
    fireEvent.blur(ids[0] as HTMLInputElement)
    expect((ids[0] as HTMLInputElement).value).toBe('hiveforge-v4-flash')
    ...
    expect(validateHiveForgeModels([{ id: '   ' }])).toEqual({ index: 0, key: 'modelIdRequired' })
    expect(validateHiveForgeModels([{ id: 'model' }, { id: 'model ' }]))
      .toEqual({ index: 1, key: 'modelIdDuplicate' })
  })

  it('renders malformed draft fallbacks without inventing catalog values', () => {
    render(<HiveForgeModelsEditor
      models={[{}]}
      overridden={false}
  ...
  it('can empty and reset the model override, then clear optional fields without dropping hidden data', async () => {
    ...
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ...
      ops: [{
        ...
        path: ['models'],
        value: [
          { id: 'hiveforge-v4-flash', description: 'Preserved hidden detail' },
          DEFAULT_HIVEFORGE_MODELS[1],
        ],
      }],
      expectedRevision: 0,
    })
  ...
  it('clears an inherited override with an unset op, never a whole-section replace', async () => {
    ...
    const { replace, update, mutate } = await mountHiveForgeCard()
    fireEvent.click(screen.getByText(en.customized))
  ...
  it('pins the hiveforge placeholder and clears typed input back to inherited', async () => {
    const { face } = scriptedFace()
  ...
  it('rejects an invalid draft before writing', async () => {
    const { update } = await mountHiveForgeCard()
    fireEvent.click(screen.getByText(en.customized))
  ...
  it('tells the user to reopen when another writer moved the namespace first', async () => {
    ...
    const { set } = await mountHiveForgeCard({
  ...
  it('keeps the card usable when the write rejects instead of answering', async () => {
    ...
    await mountHiveForgeCard({ mutate: vi.fn(() => Promise.reject(new Error('connection lost'))) })
    fireEvent.click(screen.getByText(en.customized))
  ...
  it('surfaces a shadowed credential write on the card', async () => {
    await mountFirstRun({
      set: vi.fn(() => Promise.resolve(fail('credentials: HIVEFORGE_API_KEY is shadowed by the read-only environment', 'credential-rejected'))),
    })
    const key = screen.getByLabelText<HTMLInputElement>(en.keyInput)
    fireEvent.change(key, { target: { value: 'sk-live' } })
    fireEvent.click(screen.getByText(en.apply))
  ...
  it('collapses the setup card on cancel without disturbing another open card', async () => {
    ...
    // …and HiveForge collapsed to an ordinary row carrying the missing-key dot.
    expect(screen.getAllByLabelText(en.keyInput)).toHaveLength(1)
...
describe('apiKeyFailure', () => {
  ...
  it.each([
    ['a pasted environment line', 'HIVEFORGE_API_KEY=sk-abc'],
