 ...
 * @module @hiveforge-ai/dsh-app-boot
 */
...
import { Context, type FiberState } from '@hiveforge-ai/cordis'
import Loader, { type Entry, type EntryOptions } from '@hiveforge-ai/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@hiveforge-ai/cordis-plugin-include'
import Group from '@hiveforge-ai/cordis-plugin-group'
import { dshHomePath, resolveDshHome } from '@hiveforge-ai/dsh-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@hiveforge-ai/dsh-launch-environment'
import type {} from '@hiveforge-ai/cordis-plugin-hmr'
...
const BOOTSTRAP_NAMES = new Set([
  ...
  'HIVEFORGE_BASE_URL', 'HIVEFORGE_SEARCH_BASE_URL',
 ...
 * entries (`@hiveforge-ai/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@hiveforge-ai/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@hiveforge-ai/cordis-plugin-group`
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The HiveForge implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend the implementation itself.`,
  })
}
