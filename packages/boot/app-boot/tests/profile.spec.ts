...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@hiveforge-ai/dsh-base'])
    ...
    expect(readProfileManifest('t', dir).dsh?.profile?.bundles).toEqual(['@hiveforge-ai/dsh-base'])
...
describe('loadProfile', () => {
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @hiveforge-ai/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@hiveforge-ai/dsh-base')
    try {
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@hiveforge-ai/dsh-base': { patch: '[]\n' },
      '@hiveforge-ai/dsh-web-app': { patch: '[]\n' },
      '@hiveforge-ai/dsh-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@hiveforge-ai/dsh-base', '@hiveforge-ai/dsh-web-app', '@hiveforge-ai/dsh-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).dsh?.profile?.bundles)
      .toEqual(['@hiveforge-ai/dsh-base', '@hiveforge-ai/dsh-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@hiveforge-ai/dsh-base', '@hiveforge-ai/dsh-web-app', '@hiveforge-ai/dsh-headless', 'custom-bundle',
    ])
