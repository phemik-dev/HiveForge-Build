/**
 * HiveForge search through an Anthropic-compatible Messages model call with the native
 ...
 * @module @hiveforge-ai/dsh-web-search-hiveforge/provider
 */
...
export const HIVEFORGE_PROVIDER_ID = 'hiveforge-official'

/**
 * Default endpoint: HiveForge's Anthropic-compatible API, `/v1` included
 * (`/messages` is appended). This is NOT the chat-completions base
 * (`https://api.hiveforge.com`) `@hiveforge-ai/dsh-llm-hiveforge` uses, so this
 * provider does NOT reuse `$HIVEFORGE_BASE_URL` — only the API key is shared.
 */
export const HIVEFORGE_DEFAULT_BASE_URL = 'https://api.hiveforge.com/anthropic/v1'

/** Default Anthropic-format model name (aligned with the repo's HiveForge model vocabulary). */
export const HIVEFORGE_DEFAULT_MODEL = 'hiveforge-v4-flash'

/** Default `anthropic-version` header value. */
export const HIVEFORGE_DEFAULT_API_VERSION = '2023-06-01'

/** Default upper bound on generated tokens for the Messages request. */
export const HIVEFORGE_DEFAULT_MAX_TOKENS = 4096

/** Default maximum `web_search` server-tool uses per request. */
export const HIVEFORGE_DEFAULT_MAX_USES = 5

/** Attribution header sent on every request. Bump with the package version. */
const USER_AGENT = 'hiveforge-harness/0.0.1'

/**
 * Exact secret-free HiveForge Messages request recorded immediately before one
 * auxiliary search dispatch.
 */
export interface HiveForgeSearchLlmRequest {
...
export interface HiveForgeSearchProviderOptions {
  /** Literal HiveForge API key; when present it wins over {@link resolveApiKey}. */
  apiKey?: string
  /** Resolve the current HiveForge API key for one search operation. */
   ...
   */
  recordRequest?: (request: HiveForgeSearchLlmRequest) => void
}
...
/**
 * Map a HiveForge Anthropic Messages response to a normalized search result. Walks
...
export function mapAnthropicResponse(response: AnthropicResponse): WebSearchResult {
  ...
  if (resultBlocks.length === 0) {
    throw new WebError(
      'HiveForge returned no web_search_tool_result blocks; the request may not have triggered native web search',
      'WEB_PROVIDER_ERROR',
    )
  }
...
/** The HiveForge-backed search provider; HTTP redirects fail as `WEB_PROVIDER_ERROR`. */
export class HiveForgeSearchProvider implements WebSearchProvider {
  readonly id = HIVEFORGE_PROVIDER_ID
   ...
   */
  constructor(private readonly resolveOptions: () => HiveForgeSearchProviderOptions) {}
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    const body: HiveForgeSearchLlmRequest['body'] = {
      model: options.model,
    ...
    try {
      response = await fetch(endpoint, {
        ...
        redirect: 'error',
        headers: {
          // Official HiveForge expects `x-api-key`; an Anthropic-compatible proxy
      ...
      throw new WebError(`HiveForge search request failed: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error })
    }

    if (!response.ok) {
      const status = response.status
      let message = `HiveForge API error (HTTP ${status})`
      try {
    ...
    try {
      ...
      throw new WebError(`HiveForge returned an unprocessable response body: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error })
    }
  }
   ...
   */
  private async apiKey(options: HiveForgeSearchProviderOptions, signal?: AbortSignal): Promise<string> {
    throwIfSearchAborted(signal)
    ...
    try {
      ...
      if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal, error)
      throw new WebError(
        `HiveForge search credential resolution failed: ${String(error)}`,
        'WEB_PROVIDER_ERROR',
        { cause: error },
      )
    }
    if (resolved !== undefined && resolved.length > 0) return resolved
    const ref = options.apiKeyEnv ?? 'HIVEFORGE_API_KEY'
    throw new WebError(
      `HiveForge search has no API key for "${ref}"; store it through the credentials service`
      + ' (the web Models page writes it), export it in the launching environment, or set a literal'
      + ' "apiKey" in the web-search-hiveforge config',
...
function searchAborted(signal?: AbortSignal, fallback?: unknown): WebError {
  return new WebError('HiveForge search aborted', 'WEB_ABORTED', {
...
/** True for HiveForge request limits that can be sent to the Messages API. */
