/** The `web-search-hiveforge` settings section layered over the composition entry. */

import { afterEach, describe, expect, it, vi } from 'vitest'
import { Context } from '@hiveforge-ai/cordis'
import type { Fiber } from '@hiveforge-ai/cordis'
import { SettingsProvider } from '@hiveforge-ai/dsh-settings'
...
import { WEB_SEARCH_HIVEFORGE_SETTINGS_NAMESPACE } from '@hiveforge-ai/dsh-web-search-hiveforge'
...
async function boot(): Promise<{ ctx: Context; settingsFiber: Fiber; pluginFiber: Fiber }> {
  ...
  const pluginFiber = ctx.plugin(hiveforgePlugin, { apiKey: 'ds-key', baseURL: 'https://search.entry.test/v1' })
...
describe('web-search-hiveforge settings section', () => {
  it('serves a stored endpoint to the next search without re-registering the provider', async () => {
    ...
    await bench.ctx.settings.update(WEB_SEARCH_HIVEFORGE_SETTINGS_NAMESPACE, {
  ...
  it('keeps the literal key out of every described layer', async () => {
    const bench = await boot()
    await bench.ctx.settings.update(WEB_SEARCH_HIVEFORGE_SETTINGS_NAMESPACE, { apiKey: 'ds-stored-secret' })

    const [descriptor] = bench.ctx.settings.describe({ redactSecrets: true })
      .filter(row => String(row.ns) === 'web-search-hiveforge')
  ...
  it('releases the namespace when the plugin unloads', async () => {
    const bench = await boot()
    expect(bench.ctx.settings.describe().map(row => String(row.ns))).toContain('web-search-hiveforge')

    await bench.pluginFiber.dispose()

    expect(bench.ctx.settings.describe().map(row => String(row.ns))).not.toContain('web-search-hiveforge')
    await bench.ctx.fiber.dispose()
  })
})
