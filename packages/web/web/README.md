...
| Package | Role |
|---|---|
| `@hiveforge-ai/dsh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@hiveforge-ai/dsh-web-search-exa` | Search provider: Exa |
| `@hiveforge-ai/dsh-web-search-perplexity` | Search provider: Perplexity |
| `@hiveforge-ai/dsh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@hiveforge-ai/dsh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
