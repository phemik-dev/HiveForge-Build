...
import { assertUsableApiKey, INVALID_CREDENTIAL_CODE, normalizeApiKey } from '@hiveforge-ai/dsh-llm'
...
describe('assertUsableApiKey', () => {
  it('returns the trimmed key when it is usable', () => {
    expect(assertUsableApiKey('  sk-abc  ', 'llm-hiveforge', 'HIVEFORGE_API_KEY')).toBe('sk-abc')
  })

  it('refuses a blank stored credential, naming the reference', () => {
    expect(() => assertUsableApiKey('   ', 'llm-hiveforge', 'HIVEFORGE_API_KEY'))
      .toThrow(/llm-hiveforge: the API key resolved from HIVEFORGE_API_KEY is blank/)
  })
  ...
  it('never echoes the key it refuses', () => {
    try {
      assertUsableApiKey('sk-\u{1F600}supersecret', 'llm-hiveforge', 'HIVEFORGE_API_KEY')
      expect.fail('an illegal key must throw')
    } catch (error) {
