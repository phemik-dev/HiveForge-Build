/** OpenAI-compatible HiveForge Files API transport. @module dsh-llm-hiveforge/files-api */

import { attributionHeaders, LlmError } from '@hiveforge-ai/dsh-llm'
import type { ImageMediaType } from '@hiveforge-ai/dsh-attachment'
import { HiveForgeFileId } from './file-id.ts'
import type { HiveForgeFileId as HiveForgeFileIdType } from './file-id.ts'
...
export interface HiveForgeFileObject {
  id: HiveForgeFileIdType
  bytes: number
  createdAt: number
  filename: string
  purpose: 'user_data'
...
export interface HiveForgeFilePage {
  data: HiveForgeFileObject[]
  firstId?: HiveForgeFileIdType
  lastId?: HiveForgeFileIdType
  hasMore: boolean
}

/** Files API operation failure with its HTTP status retained for recovery policy. */
export class HiveForgeFilesError extends LlmError {
 ...
 */
export function isFilesQuotaError(error: unknown): error is HiveForgeFilesError {
  return error instanceof HiveForgeFilesError
...
function invalidResponse(operation: string): LlmError {
  return new LlmError(`HiveForge Files API returned an invalid ${operation} response.`, 'INVALID_RESPONSE')
}

function parseFileObject(value: unknown, operation: string): HiveForgeFileObject {
...
export class HiveForgeFilesClient {
  ...
  private async request(path: string, init: RequestInit, signal?: AbortSignal): Promise<Response> {
    ...
    try {
      ...
      throw new LlmError(`HiveForge Files API request to ${this.baseURL} failed`, 'TRANSPORT', { cause: error })
    }
    ...
    throw new HiveForgeFilesError(
      message ?? `HiveForge Files API error (HTTP ${response.status})`,
      response.status,
      detail,
    )
  }
  ...
  async upload(input: {
    ...
    signal?: AbortSignal
  }): Promise<HiveForgeFileObject & { expiresAt: number }> {
    if (input.data.byteLength > MAX_FILE_UPLOAD_BYTES) {
      throw new LlmError('HiveForge Files API upload exceeds 128 MiB.', 'INVALID_REQUEST')
    }
    if (!Number.isSafeInteger(input.expiresAfterSeconds)
      ...
      throw new LlmError('HiveForge file expiry must be between 3600 and 2592000 seconds.', 'INVALID_REQUEST')
    }
   ...
   */
  async list(options: {
    after?: HiveForgeFileIdType
    limit?: number
    order?: 'asc' | 'desc'
    signal?: AbortSignal
  } = {}): Promise<HiveForgeFilePage> {
    ...
    return {
      ...
      ...typeof wire.first_id === 'string' ? { firstId: HiveForgeFileId(wire.first_id) } : {},
      ...typeof wire.last_id === 'string' ? { lastId: HiveForgeFileId(wire.last_id) } : {},
   ...
   */
  async retrieve(fileId: HiveForgeFileIdType, signal?: AbortSignal): Promise<HiveForgeFileObject> {
   ...
   * @param signal - request cancellation.
   */
  async delete(fileId: HiveForgeFileIdType, signal?: AbortSignal): Promise<void> {
