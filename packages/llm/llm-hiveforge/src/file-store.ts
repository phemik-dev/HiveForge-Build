/** HiveForge Files API upload reuse, invalidation, and quota recovery. @module dsh-llm-hiveforge/file-store */

import type { RequestImageAttachment } from '@hiveforge-ai/dsh-attachment'
import { LlmError } from '@hiveforge-ai/dsh-llm'
import { HiveForgeFilesClient, isFilesQuotaError } from './files-api.ts'
import type { HiveForgeFileId } from './file-id.ts'
import { hiveForgeFileScope, HiveForgeUploadIndex } from './upload-index.ts'
import type { HiveForgeUploadRecord } from './upload-index.ts'

/** HiveForge chat accepts at most 32 MiB per image even when it is referenced by file id. */
...
export interface HiveForgeFilePolicy {
...
export interface HiveForgeFileConnection {
  baseURL: string
  apiKey: string
}

/** Result of one file-id resolution. */
export interface HiveForgeFileReference {
  record: HiveForgeUploadRecord
  uploaded: boolean
}

interface FileStoreOptions {
  index?: HiveForgeUploadIndex
  now?: () => number
  fetch?: typeof fetch
}
...
interface SharedUpload {
  ...
  promise: Promise<HiveForgeFileReference>
  settled: boolean
  waiters: number
}
...
function abortReason(signal: AbortSignal): Error {
  ...
  return reason instanceof Error
    ? reason
    : new Error('HiveForge file upload cancelled with a non-Error reason.', { cause: reason })
}

function uploadFailure(error: unknown): Error {
  return error instanceof Error
    ? error
    : new Error('HiveForge file upload failed with a non-Error reason.', { cause: error })
}

function waitForUpload(operation: SharedUpload, signal: AbortSignal | undefined): Promise<HiveForgeFileReference> {
  ...
  return new Promise<HiveForgeFileReference>((resolve, reject) => {
    const abort = (): void => {
...
/** User-scoped durable file-id reuse for the HiveForge route. */
export class HiveForgeFileStore {
  private readonly index: HiveForgeUploadIndex
  ...
  constructor(options: FileStoreOptions = {}) {
    this.index = options.index ?? new HiveForgeUploadIndex()
    this.now = options.now ?? Date.now
    this.fetchImpl = options.fetch
  }

  private client(connection: HiveForgeFileConnection): HiveForgeFilesClient {
    return new HiveForgeFilesClient({
  ...
  ensureUploaded(
    ...
    connection: HiveForgeFileConnection,
    policy: HiveForgeFilePolicy,
    signal?: AbortSignal,
  ): Promise<HiveForgeFileReference> {
    signal?.throwIfAborted()
    const scope = hiveForgeFileScope(connection.baseURL, connection.apiKey)
  ...
  private async ensureUploadedOnce(
    ...
    if (version.bytes > MAX_CHAT_IMAGE_BYTES) {
      throw new LlmError('HiveForge chat image exceeds the 32 MiB per-image limit.', 'INVALID_REQUEST')
    }
    const scope = hiveForgeFileScope(connection.baseURL, connection.apiKey)
    const now = this.now()
    ...
    const upload = async (): Promise<HiveForgeUploadRecord> => {
      ...
      if (remote.bytes !== version.data.byteLength) {
        throw new LlmError('HiveForge Files API upload response does not match the submitted image.', 'INVALID_RESPONSE')
      }
      return {
        scope,
    ...
    let candidate: HiveForgeUploadRecord
    try {
  ...
  async invalidate(
    ...
    await this.index.remove(
      hiveForgeFileScope(connection.baseURL, connection.apiKey),
      version.variantId,
      fileId,
    )
  }
  ...
  async reclaimOldestOwned(
    ...
    const client = this.client(connection)
    let after: HiveForgeFileId | undefined
    const owned: HiveForgeFileId[] = []
   ...
   */
  async releaseAll(connection: HiveForgeFileConnection, signal?: AbortSignal): Promise<number> {
    let total = 0
    for (;;) {
    ...
    await this.index.clear(hiveForgeFileScope(connection.baseURL, connection.apiKey))
    return total
  }
}
