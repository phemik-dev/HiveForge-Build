/**
 * `HiveForgeAdapter`: fetch + SSE against a HiveForge (OpenAI-compatible)
 * chat-completions endpoint, emitting harness StreamChunks. The adapter is
 * transport-only: connection facts arrive through a thunk resolved once per
 * operation and the bearer token through a per-request resolver, so the
 * registering plugin owns validation, layering, and credential policy.
 *
 * @module dsh-llm-hiveforge/adapter
 */

import { attributionHeaders, contentHasImage, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, LlmAdapter, LlmError, offloadRequestImagesWithPolicy, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId } from '@hiveforge-ai/dsh-llm'
import type {
  ContentBlock,
  GenerateOptions,
  LlmModelInfo,
...
import type { CredentialRef } from '@hiveforge-ai/dsh-credentials'
import { deadline, idleWatchdog, timeoutOf } from '@hiveforge-ai/dsh-timeout'
import type { AnonymousUserId } from '@hiveforge-ai/dsh-anonymous-user-id'
import { serializeRequest, serializeRequestWithImages } from './serialize.ts'
import type { ImageWireLocation, RequestDefaults } from './serialize.ts'
import { HiveForgeFileStore } from './file-store.ts'
import type { HiveForgeFilePolicy } from './file-store.ts'
import type { HiveForgeFileId } from './file-id.ts'
 ...
 */
export interface HiveForgeConnectionOptions {
  ...
  models: readonly HiveForgeCatalogModel[]
  ...
  filePolicy: HiveForgeFilePolicy
  /** Provider-owned model-request retry policy, already resolved. */
  retryPolicy: ResolvedRetryPolicy
}

/** Constructor options for {@link HiveForgeAdapter}: the operation-local resolution hooks the plugin owns. */
export interface HiveForgeAdapterOptions {
  /** Current validated connection facts; called once per operation. */
  options: () => HiveForgeConnectionOptions
  /**
   ...
   */
  resolveApiKey: (connection: HiveForgeConnectionOptions) => Promise<string>
  ...
  resolveFiles?: () => HiveForgeFileStore
}
...
/** Total-pixel budget matching HiveForge's normal vision projection. */
...
const FILES_API_TIMEOUT_CODE = 'HIVEFORGE_FILES_API_TIMEOUT'
...
class FileResolutionFailure extends Error {
  constructor(cause: unknown) {
    super('HiveForge Files API could not resolve a request image.', { cause })
...
/**
 * Resolve the request-image budgets owned by one HiveForge model route.
 * @param model - Advertised model route and its optional image overrides.
 * @returns Complete pixel and encoded-byte budgets.
 * @internal
 */
export function resolveRequestImagePolicy(model: HiveForgeCatalogModel): ImageRequestPolicy {
...
function detailNamesFileId(detail: string, fileId: HiveForgeFileId): boolean {
...
function normalizedImageDiagnostic(
  ...
  if (target !== undefined) {
    return `HiveForge rejected normalized image ${normalizedImageFacts(target)}: ${providerMessage}. `
  ...
  return `HiveForge rejected a normalized request image: ${providerMessage}. Candidate images: `
    + `${candidates.map(normalizedImageFacts).join('; ')}. `
    + 'The provider rejected bytes already normalized by the harness; PNG, JPEG, WebP, and GIF remain supported input formats.'
}

function modelInfo(provider: string, model: HiveForgeCatalogModel): LlmModelInfo {
  return {
    provider,
    id: model.id,
...
function requestId(headers: Headers): ReturnType<typeof ProviderRequestId> | undefined {
  const value = headers.get('x-request-id') ?? headers.get('x-hiveforge-request-id')
 ...
 */
export class HiveForgeAdapter extends LlmAdapter {
  private readonly files: HiveForgeFileStore

  constructor(private readonly config: HiveForgeAdapterOptions) {
    super()
    this.files = config.resolveFiles?.() ?? new HiveForgeFileStore()
  }
  ...
  private modelInfoFor(
    connection: HiveForgeConnectionOptions,
    provider: string,
    model: string,
  ): LlmResolvedModelInfo {
  ...
  private async * streamWithConnection(
    ...
    if (hasImages) {
      ...
      if (model?.inputModalities?.includes('image') !== true) {
        throw new LlmError(
          `HiveForge model "${options.model}" does not accept image input.`,
          'UNSUPPORTED_CONTENT',
        )
      }
      ...
      if (attachments === undefined) {
        throw new LlmError(
          'HiveForge image conversion requires the durable attachment service.',
          'UNSUPPORTED_CONTENT',
        )
      }
    }
    ...
    try {
      ...
      if (timeoutOf(watchdog.signal, STREAM_IDLE_TIMEOUT_CODE) !== undefined) {
        throw new LlmError(
          `HiveForge stream idle timeout after ${connection.streamIdleTimeoutMs}ms`,
          'TIMEOUT',
          { cause: error },
        )
      }
      if (options.signal?.aborted) {
        throw new LlmError('HiveForge request aborted by caller', 'ABORTED', { cause: error })
      }
      if (error instanceof LlmError) throw error
      throw new LlmError(`HiveForge API stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    } finally {
      consumer.abort('HiveForge stream consumer stopped')
  ...
  private async * request(
    ...
    const headers = {
      ...
      ...attributionHeaders(),
      'x-hiveforge-harness-user-id': String(userId),
      ...options.sessionId !== undefined
        ? { 'x-hiveforge-harness-session-id': String(options.sessionId) }
        : {},
      ...options.purpose === 'compaction'
        ? { 'x-hiveforge-harness-compact': '1' }
        : {},
    }
    ...
    while (true) {
          ...
          body = await serializeRequestWithImages(requestOptions, {
            representation: {
              ...
              resolveFileId: async (version, _block, location) => {
                ...
                let resolved: Awaited<ReturnType<HiveForgeFileStore['ensureUploaded']>>
                try {
      ...
      try {
        ...
        if (signal.aborted) throw error
        throw new LlmError(
          `HiveForge API request to ${connection.baseURL} failed`,
          'TRANSPORT',
          { cause: error },
        )
      }
      ...
      if (!response.ok) {
        let message = `HiveForge API error (HTTP ${response.status})`
        ...
        throw new LlmError(message, httpErrorCode(response.status, providerError), {
          cause: new Error(rawResponse.length > 0 ? rawResponse : `HiveForge HTTP ${response.status}`),
      ...
      if (!response.body) {
        throw new LlmError('HiveForge API returned no response body', 'EMPTY_RESPONSE')
      }
