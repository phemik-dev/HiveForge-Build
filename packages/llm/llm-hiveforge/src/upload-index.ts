/** Durable HiveForge attachment-to-file-id index. @module dsh-llm-hiveforge/upload-index */
...
import { ImageVariantId } from '@hiveforge-ai/dsh-attachment'
import type { AttachmentId, ImageVariantId as ImageVariantIdType } from '@hiveforge-ai/dsh-attachment'
import { HiveForgeFileId, HiveForgeFileScope } from './file-id.ts'
import type { HiveForgeFileId as HiveForgeFileIdType, HiveForgeFileScope as HiveForgeFileScopeType } from './file-id.ts'

/** One durable remote upload mapping. Unix times are milliseconds. */
export interface HiveForgeUploadRecord {
  scope: HiveForgeFileScopeType
 ...
 */
export function hiveForgeFileScope(baseURL: string, apiKey: string): HiveForgeFileScopeType {
  const digest = createHash('sha256')
    .update(baseURL.replace(/\/+$/u, ''))
    .update('\0')
    .update(apiKey)
    .digest('hex')
  return HiveForgeFileScope(digest)
}
...
function parseRecord(value: unknown): HiveForgeUploadRecord {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-hiveforge: upload index contains a non-object record')
  }
  ...
  if (typeof record.scope !== 'string' || !/^[0-9a-f]{64}$/u.test(record.scope)
    ...
    throw new InvalidUploadIndexError('llm-hiveforge: upload index contains an invalid record')
  }
  return {
    scope: HiveForgeFileScope(record.scope),
    attachmentId: record.attachmentId as AttachmentId,
    variantId: ImageVariantId(record.variantId),
    fileId: HiveForgeFileId(record.fileId),
...
function parseIndex(text: string): StoredIndex {
  ...
  try {
    ...
    throw new InvalidUploadIndexError('llm-hiveforge: upload index is not valid JSON', { cause: error })
  }
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-hiveforge: upload index is not an object')
  }
  const index = value as { formatVersion?: unknown; records?: unknown }
  if (index.formatVersion !== 3 || !Array.isArray(index.records)) {
    throw new InvalidUploadIndexError('llm-hiveforge: unsupported upload index format')
  }
  const records = index.records.map(parseRecord)
  const keys = new Set<string>()
  for (const record of records) {
    const key = `${record.scope}\0${record.variantId}`
    if (keys.has(key)) throw new InvalidUploadIndexError('llm-hiveforge: upload index contains duplicate mappings')
    keys.add(key)
  }
  return { formatVersion: 3, records }
}

function reusable(record: HiveForgeUploadRecord, now: number, refreshMarginMs: number): boolean {
  return record.expiresAt - now > refreshMarginMs
}

/** Atomic local index shared by every HiveForge session in this DSH home. */
export class HiveForgeUploadIndex {
  /** Absolute owner-private JSON index path. */
  readonly path: string

  /**
   * @param path - explicit test path; omission uses `DSH_HOME/llm-hiveforge/files-v3.json`.
   */
  constructor(path = join(resolveDshHome(), 'llm-hiveforge', 'files-v3.json')) {
    this.path = path
  }
  ...
  ): Promise<HiveForgeUploadRecord | undefined> {
   ...
   */
  async commit(
    candidate: HiveForgeUploadRecord,
    now: number,
    refreshMarginMs: number,
   ...
   * @param scope - endpoint/API-key namespace.
   */
  async clear(scope: HiveForgeFileScopeType): Promise<void> {
    await mkdir(dirname(this.path), { recursive: true, mode: 0o700 })
    await withFileLock(this.path, async () => {
      const index = await this.load()
