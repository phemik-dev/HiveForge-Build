/**
 * Serialize harness messages into HiveForge chat completions. Text-only
 ...
 * @module dsh-llm-hiveforge/serialize
 */

import { contentHasImage, LlmError, offloadRequestImagesWithPolicy, requestImageHandleText } from '@hiveforge-ai/dsh-llm'
import type { ContentBlock, GenerateOptions, Message } from '@hiveforge-ai/dsh-llm'
import type { ImageAttachmentRef, RequestImageAttachment } from '@hiveforge-ai/dsh-attachment'
import type {
...
export type ImageRequestRepresentation =
  | {
    kind: 'file'
    /** Resolve a retained request version to a reusable HiveForge file id. */
    resolveFileId: (
...
/** Validate the adapter-owned effort before resolving its HiveForge wire fields. */
function reasoningEffort(effort: NonNullable<GenerateOptions['reasoningEffort']>): 'off' | 'low' | 'high' | 'max' {
  ...
  throw new LlmError(
    `HiveForge does not support reasoning effort "${effort}"`,
...
function resolveThinking(options: GenerateOptions, defaults: RequestDefaults): ResolvedThinking {
  ...
  if (defaults.thinking === 'disabled' && effort !== undefined && effort !== 'off') {
    throw new LlmError(
      `HiveForge deployment does not support reasoning effort "${effort}"`,
      'UNSUPPORTED_REASONING_EFFORT',
    )
  }
...
function assertTextOnly(blocks: readonly ContentBlock[]): void {
  if (contentHasImage(blocks)) {
    throw new LlmError('The HiveForge chat-completions adapter does not support image content.', 'UNSUPPORTED_CONTENT')
  }
}

/** Reject roles whose HiveForge history format cannot carry image input. */
function assertSupportedImageRoles(messages: readonly Message[]): void {
  for (const message of messages) {
    if (message.role !== 'user' && contentHasImage(message.content)) {
      throw new LlmError(
        `The HiveForge chat-completions adapter cannot represent image content in a ${message.role} message.`,
        'UNSUPPORTED_CONTENT',
      )
    }
  }
...
/** Resolve one durable image into its descriptor and transient HiveForge image part. */
async function imageParts(
  ...
  if (version === undefined) {
    throw new LlmError(
      `HiveForge request image ${block.attachment.attachmentId} was not prepared.`,
      'INVALID_REQUEST',
    )
  }
  ...
  for (const block of blocks) {
    switch (block.type) {
      ...
      case 'tool-result':
        ...
        break
      default:
        // Other merge-extensible blocks are not HiveForge user-input vocabulary.
        break
    }
  }
  return parts
...
export function serializeMessages(messages: Message[]): WireMessage[] {
  ...
  for (const message of messages) {
    ...
    // vocabulary, but HiveForge wants them as role:'tool' messages.
 ...
 * @returns ordered HiveForge wire messages.
 */
 ...
 * @returns the fully materialized HiveForge request body.
 */
export async function serializeRequestWithImages(
  ...
  const requestMessages = offloadRequestImagesWithPolicy(options.messages, {
    ...
    byteLength: (ref) => {
      ...
      if (version === undefined) {
        throw new LlmError(`HiveForge request image ${ref.attachmentId} was not prepared.`, 'INVALID_REQUEST')
      }
