 ...
 * settings-file, credentials-local, and llm-hiveforge boot from a test-only
...
import { getOrCreateAnonymousUserId } from '@hiveforge-ai/dsh-anonymous-user-id'
...
async function loadComposition(
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@hiveforge-ai/dsh-llm-hiveforge', LlmHiveForge],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-hiveforge real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    expect(serverA.headers[0]?.['x-hiveforge-harness-user-id']).toBe(getOrCreateAnonymousUserId())

    // External edits, exactly as a user or the web UI would leave them on disk.
    await writeFile(settingsPath, `llm-hiveforge:\n  baseURL: ${serverB.url}\n`)
    await vi.waitFor(() => {
      expect((ctx.get('settings')!.get(NS) as { baseURL?: string }).baseURL).toBe(serverB.url)
    }, { timeout: 5000 })
    await writeFile(credentialsPath, 'version: 1\nrefs:\n  HIVEFORGE_API_KEY: rotated-key\n', { mode: 0o600 })
  ...
  it('keeps a stored key writable and rotatable across a real restart', async () => {
    // No ambient HIVEFORGE_API_KEY: the shipped surfaces do not hoist
    ...
    await assemble(boot.ctx, { model: 'hiveforge-v4-flash', messages: [] })
    ...
    await assemble(restarted.ctx, { model: 'hiveforge-v4-flash', messages: [] })
  ...
  it('boots the same adapter on entry config alone, resolving the reference from the environment', async () => {
    ...
    vi.stubEnv('HIVEFORGE_API_KEY', 'entry-key')
