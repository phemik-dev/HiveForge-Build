...
import LlmRuntime, { createUserMessage, CallId, ReasoningEffortId, createMessage } from '@hiveforge-ai/dsh-llm'
import type { Message, ToolSchema } from '@hiveforge-ai/dsh-llm'
import AttachmentStore, { AttachmentId, ImageVariantId } from '@hiveforge-ai/dsh-attachment'
import type {
...
import type { Config } from '@hiveforge-ai/dsh-llm-hiveforge'
 ...
 * without $HIVEFORGE_API_KEY; the pre-release vision smoke additionally
 * requires $HIVEFORGE_VISION_E2E=1 (see vitest.e2e.config.ts).
 */

const FLASH = 'hiveforge-v4-flash'
const PRO = 'hiveforge-v4-pro'
const VISION = 'hiveforge-v4-flash-vision-exp'
const VISION_E2E_ENABLED = process.env.HIVEFORGE_VISION_E2E === '1'
...
async function harness(_model: string, config: Partial<Config> = {}) {
  ...
  await ctx.plugin(LlmHiveForge, config)
  return ctx
}
...
describe.skipIf(!process.env.HIVEFORGE_API_KEY)('llm-hiveforge e2e (real API)', () => {
  it.skipIf(!VISION_E2E_ENABLED)('uses the built-in official route to upload, reference, and delete one image', async () => {
    const key = process.env.HIVEFORGE_API_KEY
    if (key === undefined) throw new Error('e2e ran without HIVEFORGE_API_KEY')
    const baseURL = process.env.HIVEFORGE_BASE_URL ?? LlmHiveForge.PUBLIC_BASE_URL
    const ctx = await harness(VISION, { baseURL })
    await ctx.plugin(E2eAttachmentStore)
    const attachments = ctx.attachments as E2eAttachmentStore
    let uploadedFile: LlmHiveForge.HiveForgeFileIdType | undefined
    ...
    const observedFetch: typeof fetch = async (input, init) => {
      ...
      if (method === 'POST' && url.pathname.endsWith('/files') && response.ok) {
        ...
        if (typeof value.id === 'string') uploadedFile = LlmHiveForge.HiveForgeFileId(value.id)
      }
      return response
    }
    vi.stubGlobal('fetch', observedFetch)
    const files = new LlmHiveForge.HiveForgeFilesClient({ baseURL, apiKey: key })
    ...
    try {
      ...
      expect(
        result.finish.kind,
        `HiveForge vision result: ${JSON.stringify(result.finish)}`,
      ).toBe('stop')
