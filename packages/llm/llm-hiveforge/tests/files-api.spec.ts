...
import { userAgent } from '@hiveforge-ai/dsh-llm'
import { HiveForgeFileId } from '../src/file-id.ts'
import {
  HiveForgeFilesClient,
  HiveForgeFilesError,
  isFilesQuotaError,
...
describe('HiveForgeFilesClient', () => {
  it('uploads multipart bytes with the required purpose and explicit expiry', async () => {
    ...
    const client = new HiveForgeFilesClient({ baseURL: 'https://api.hiveforge.com/', apiKey: 'key', fetch: fetchImpl })
    ...
    await expect(client.upload({
      ...
      id: HiveForgeFileId('file-api-one'),
      bytes: 3,
  ...
  it('validates list, retrieve, and delete responses', async () => {
    ...
    await expect(client.list({ after: HiveForgeFileId('file-api-before'), limit: 20, order: 'desc' })).resolves.toMatchObject({
      data: [{ id: 'file-api-one' }], firstId: 'file-api-one', lastId: 'file-api-one', hasMore: false,
    })
    await expect(client.retrieve(HiveForgeFileId('file-api-one'))).resolves.toMatchObject({ id: 'file-api-one' })
    await expect(client.delete(HiveForgeFileId('file-api-one'))).resolves.toBeUndefined()
  })
  ...
  it.each([
    ...
    const client = new HiveForgeFilesClient({
      baseURL: 'https://api.hiveforge.com',
      apiKey: 'key',
      fetch: vi.fn(() => Promise.resolve(new Response('not-json', { status }))),
    })
    await expect(client.retrieve(HiveForgeFileId('missing'))).rejects.toMatchObject({
      name: 'HiveForgeFilesError',
      code,
      detail: '',
    })
  })
  ...
  it.each([
    ...
    const error = await client.retrieve(HiveForgeFileId('missing')).catch((caught: unknown) => caught)
    expect(error).toBeInstanceOf(HiveForgeFilesError)
    expect(error).toMatchObject({ message: 'HiveForge Files API error (HTTP 400)', detail: '' })
  })
  ...
  it('wraps transport failures but preserves an aborted request reason', async () => {
    ...
    await expect(client.retrieve(HiveForgeFileId('one'))).rejects.toMatchObject({
      code: 'TRANSPORT',
      cause: transport,
    })
    ...
    await expect(client.retrieve(HiveForgeFileId('one'), controller.signal)).rejects.toBe(transport)
  })
  ...
  it.each([
    ...
    await expect(client.retrieve(HiveForgeFileId('one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
  ...
  it('accepts a list without cursors and uses the global fetch default', async () => {
    ...
    try {
      const client = new HiveForgeFilesClient({ baseURL: 'https://api.hiveforge.com///', apiKey: 'key' })
  ...
  it.each([
    ...
    await expect(client.delete(HiveForgeFileId('file-api-one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
})
