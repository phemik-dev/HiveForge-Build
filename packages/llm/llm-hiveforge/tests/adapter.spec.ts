...
import type { AttachmentStore, ImageAttachmentRef, RequestImageAttachment } from '@hiveforge-ai/dsh-attachment'
import { createLaunchEnvironmentSnapshot } from '@hiveforge-ai/dsh-launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@hiveforge-ai/dsh-anonymous-user-id'
import { SessionId } from '@hiveforge-ai/dsh-session'
import * as LlmHiveForge from '@hiveforge-ai/dsh-llm-hiveforge'
import { HiveForgeAdapter, resolveAdapterOptions } from '@hiveforge-ai/dsh-llm-hiveforge'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'dsh-llm-hiveforge-'))
...
async function harness(baseURL: string, config: object = {}) {
  ...
  await ctx.plugin(LlmHiveForge, { baseURL, ...config })
  return ctx
}

/** Direct adapter over the plugin's real resolve step, with a static key. */
function adapterOf(
  config: Partial<LlmHiveForge.Config> & { apiKey?: string } = {},
  attachments?: AttachmentStore,
  files?: LlmHiveForge.HiveForgeFileStore,
): HiveForgeAdapter {
  const { apiKey, ...rest } = config
  return new HiveForgeAdapter({
...
function fileStoreOf(
  implementation: (...args: Parameters<LlmHiveForge.HiveForgeFileStore['ensureUploaded']>) => ReturnType<LlmHiveForge.HiveForgeFileStore['ensureUploaded']>,
) {
  const ensureUploaded = vi.fn(implementation)
  const invalidate = vi.fn(() => Promise.resolve())
  return {
    store: { ensureUploaded, invalidate } as unknown as LlmHiveForge.HiveForgeFileStore,
    ensureUploaded,
    invalidate,
  }
}

function fileReference(fileId: string): Awaited<ReturnType<LlmHiveForge.HiveForgeFileStore['ensureUploaded']>> {
  return {
    record: { fileId: LlmHiveForge.HiveForgeFileId(fileId) },
    uploaded: true,
  } as Awaited<ReturnType<LlmHiveForge.HiveForgeFileStore['ensureUploaded']>>
}
...
describe('HiveForgeAdapter against a mock server', () => {
  it('streams a text generation end to end through the assembler', async () => {
    ...
    // App attribution and HiveForge request identity are independent wire facts.
    expect(server.headers[0]?.['user-agent']).toBe(userAgent())
    expect(server.headers[0]?.['x-hiveforge-harness-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-hiveforge-harness-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-hiveforge-harness-compact')
  })
  ...
  it('uploads a durable image once and sends only its Files API id to the vision model', async () => {
    ...
    await drain(adapter.stream({
      ...
      model: 'hiveforge-v4-flash-vision-exp',
      messages: [createUserMessage({
        content: [
  ...
  it('falls back to one all-base64 request when Files API resolution fails', async () => {
    ...
    const adapter = adapterOf({
      baseURL: server.url,
      models: [{ id: 'hiveforge-v4-flash-vision-exp', inputModalities: ['text', 'image'] }],
  ...
  it('falls back after the configured Files API deadline without aborting chat', async () => {
    ...
    const adapter = adapterOf({
      baseURL: 'https://hiveforge.invalid',
  ...
  it('projects nested tool-result images with route-owned request budgets', async () => {
    ...
    await drain(adapter.stream({ provider: 'hiveforge-official', model: 'vision-low', messages: [nested] }))
    await drain(adapter.stream({ provider: 'hiveforge-official', model: 'vision-custom', messages: [nested] }))
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    await drain(adapter.stream({ provider: 'hiveforge-official', model: 'hiveforge-v4-flash-vision-exp', messages }))
    await drain(adapter.stream({
      provider: 'hiveforge-official',
      model: 'hiveforge-v4-flash-vision-exp',
      messages,
    ...
    expect(server.headers[1]?.['x-hiveforge-harness-compact']).toBe('1')
  })
  ...
  it.each(['hiveforge-v4-flash', 'unlisted-pass-through'])(
    ...
    async (model) => {
      ...
      const adapter = new HiveForgeAdapter({
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-hiveforge-harness-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-hiveforge-harness-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-hiveforge-harness-compact']).toBe('1')
  })
  ...
  it('uses the configured maxTokens default and preserves an explicit request cap', async () => {
    ...
    await assemble(ctx, { model: 'hiveforge-v4-flash', messages: [], maxTokens: 8_192 })
  ...
  it('publishes only off and omits the wire effort when thinking is disabled', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('hiveforge-official', 'hiveforge-v4-flash'))
  ...
  it('uses the HTTP status as the cause when an error response has no body', async () => {
    ...
    await expect(drain(adapter.stream({
      ...
      code: 'SERVER',
      cause: { message: 'HiveForge HTTP 500' },
    })
  })
  ...
  it('parses a future Retry-After HTTP date and the HiveForge request-id fallback', async () => {
    ...
    try {
      const server = await mockServer([{
        ...
        headers: {
          ...
          'x-hiveforge-request-id': 'hiveforge-503',
        },
      }])
      const ctx = await harness(server.url)
      const result = await assemble(ctx, { model: 'hiveforge-v4-flash', messages: [] })
      expect(result.finish).toEqual({
        ...
        failure: {
          ...
          requestId: ProviderRequestId('hiveforge-503'),
        },
      })
    } finally {
      dateNow.mockRestore()
  ...
  it('reports a transport failure with the endpoint in the message', async () => {
    ...
    expect(result.finish).toMatchObject({
      ...
      failure: {
        code: 'TRANSPORT',
        message: 'HiveForge API request to http://127.0.0.1:1 failed',
      },
    })
  })
  ...
  it('throws EMPTY_RESPONSE when the response has no body', async () => {
    ...
    try {
      const iterate = async (): Promise<void> => {
        for await (const _chunk of adapter.stream({ provider: 'hiveforge-official', model: 'm', messages: [] })) { /* drain */ }
      }
  ...
  it('classifies an abrupt body close as TRANSPORT', async () => {
    ...
    expect(result.finish.failure.message).toMatch(/^HiveForge API stream from .* failed$/)
  })
  ...
  it('renders a non-Error transport rejection without losing its cause', async () => {
    ...
    try {
      ...
      await expect(drain()).rejects.toMatchObject({
        message: 'HiveForge API request to https://example.invalid failed',
        code: 'TRANSPORT',
        cause: 'offline',
      })
    } finally {
  ...
  it('keeps an idle provider read alive through SSE comments', async () => {
    ...
    try {
      const chunks: string[] = []
      const drain = (async () => {
        for await (const chunk of adapter.stream({ provider: 'hiveforge-official', model: 'm', messages: [] })) {
...
describe('plugin registration and config', () => {
  it('keeps wire helpers off the package root', () => {
    for (const helper of [
      ...
      'mapUsage',
      'translate',
    ]) expect(LlmHiveForge).not.toHaveProperty(helper)
  })

  it('registers the hiveforge provider and unregisters on dispose (HMR safety)', async () => {
    const server = await mockServer([])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const fiber = await ctx.plugin(LlmHiveForge, {
      baseURL: server.url,
    })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'hiveforge-official', name: 'HiveForge' }])
  ...
  it('registers retryPolicy from the provider config', async () => {
    ...
    expect(ctx.llm.providerRetryPolicy('hiveforge-official')).toEqual({
      mode: 'always',
      initialDelayMs: 25,
      maxDelayMs: 100,
      jitterRatio: 0.2,
  ...
  it('owns the hiveforge provider and advertises the default models', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmHiveForge, { baseURL: 'http://127.0.0.1:1' })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'hiveforge-official', name: 'HiveForge' }])
    await expect(ctx.llm.listModels('hiveforge-official')).resolves.toEqual([
      { provider: 'hiveforge-official', id: 'hiveforge-v4-flash', name: 'HiveForge-V4-Flash', inputModalities: ['text'] },
      { provider: 'hiveforge-official', id: 'hiveforge-v4-pro', name: 'HiveForge-V4-Pro', inputModalities: ['text'] },
      { provider: 'hiveforge-official', id: 'hiveforge-v4-flash-vision-exp', name: 'HiveForge-V4-Flash-Vision-Exp', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('hiveforge-official', 'hiveforge-v4-flash'))
      .resolves.toMatchObject({
        provider: 'hiveforge-official',
    ...
    await expect(ctx.llm.resolveModelInfo('hiveforge-official', 'hiveforge-v4-flash-vision-exp'))
      .resolves.toMatchObject({
        provider: 'hiveforge-official',
        id: 'hiveforge-v4-flash-vision-exp',
        name: 'HiveForge-V4-Flash-Vision-Exp',
        inputModalities: ['text', 'image'],
        context: { contextWindow: 1_000_000 },
        defaultMaxTokens: 256_000,
      })
  ...
  it.each(['off', 'low', 'max'] as const)('uses the configured %s reasoning default', async (effort) => {
    ...
    await expect(ctx.llm.resolveModelInfo('hiveforge-official', 'unlisted-pass-through'))
      .resolves.toMatchObject({
        reasoning: {
          efforts: [
            { id: ReasoningEffortId('off'), name: 'Off' },
  ...
  it.each(['low', 'high', 'max'] as const)(
    ...
    async (reasoningEffort) => {
      const ctx = new Context()
      await ctx.plugin(LlmRuntime)
      await expect(ctx.plugin(LlmHiveForge, {
        baseURL: 'http://127.0.0.1:1',
        thinking: 'disabled',
        reasoningEffort,
  ...
  it('accepts disabled thinking with off at the resolver boundary', async () => {
    ...
    await expect(adapter.resolveModel('hiveforge-official', 'pass-through')).resolves.toMatchObject({
      reasoning: {
        efforts: [{ id: ReasoningEffortId('off'), name: 'Off' }],
        defaultEffort: ReasoningEffortId('off'),
      },
  ...
  it('uses the default model catalog when apply is called directly', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    LlmHiveForge.apply(ctx, { baseURL: 'http://127.0.0.1:1' })
    await expect(ctx.llm.listModels('hiveforge-official')).resolves.toEqual([
      { provider: 'hiveforge-official', id: 'hiveforge-v4-flash', name: 'HiveForge-V4-Flash', inputModalities: ['text'] },
      { provider: 'hiveforge-official', id: 'hiveforge-v4-pro', name: 'HiveForge-V4-Pro', inputModalities: ['text'] },
      { provider: 'hiveforge-official', id: 'hiveforge-v4-flash-vision-exp', name: 'HiveForge-V4-Flash-Vision-Exp', inputModalities: ['text', 'image'] },
  ...
  it('defaults an adapter-supplied catalog entry to text input', async () => {
    ...
    await expect(adapter.listModels('hiveforge-official')).resolves.toEqual([{
      provider: 'hiveforge-official',
      id: 'adapter-model',
      name: 'adapter-model',
  ...
  it('advertises configured models without restricting arbitrary request ids', async () => {
    ...
    await expect(ctx.llm.listModels('hiveforge-official')).resolves.toEqual([
      { provider: 'hiveforge-official', id: 'private-fast', name: 'private-fast', inputModalities: ['text'] },
      { provider: 'hiveforge-official', id: 'private-reasoner', name: 'Private Reasoner', description: 'Higher reasoning budget', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('hiveforge-official', 'private-fast'))
      .resolves.toMatchObject({ context: { contextWindow: 32_000 } })
    await expect(ctx.llm.resolveModelInfo('hiveforge-official', 'private-reasoner'))
      .resolves.toMatchObject({
        name: 'Private Reasoner',
        description: 'Higher reasoning budget',
        inputModalities: ['text', 'image'],
      })
    await expect(ctx.llm.resolveModelInfo('hiveforge-official', 'arbitrary-unlisted'))
      .resolves.toMatchObject({
        context: { contextWindow: 1_000_000 },
        defaultMaxTokens: 256_000,
      })
  ...
  it('uses exact model capacity before the adapter-wide default', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('hiveforge-official', 'inherits-default'))
      .resolves.toMatchObject({ context: { contextWindow: 256_000 } })
    await expect(ctx.llm.resolveModelInfo('hiveforge-official', 'exact-override'))
  ...
  const invalidModels: Array<[LlmHiveForge.HiveForgeCatalogModel[], RegExp]> = [
    ...
    [[{
      id: 'm',
      inputModalities: ['audio'] as unknown as NonNullable<LlmHiveForge.HiveForgeCatalogModel['inputModalities']>,
    }], /expected "text" \| "image"/],
  ...
  const invalidProgrammaticModalities: Array<[LlmHiveForge.HiveForgeCatalogModel[], RegExp]> = [
  ...
  it('prefers a model\'s own output cap over the profile default', async () => {
    ...
    await expect(adapter.resolveModel('hiveforge-official', 'capped'))
      .resolves.toMatchObject({ defaultMaxTokens: 512 })
    await expect(adapter.resolveModel('hiveforge-official', 'uncapped'))
      .resolves.toMatchObject({ defaultMaxTokens: 4096 })
    await expect(adapter.resolveModel('hiveforge-official', 'not-in-catalog'))
      .resolves.toMatchObject({ defaultMaxTokens: 4096 })
  })
  ...
  it('falls back to HIVEFORGE_API_KEY and HIVEFORGE_BASE_URL env vars', async () => {
    vi.stubEnv('HIVEFORGE_API_KEY', 'env-key')
    vi.stubEnv('HIVEFORGE_BASE_URL', 'http://127.0.0.1:1')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmHiveForge, {})
    expect(ctx.llm.listProviders()).toEqual([{ id: 'hiveforge-official', name: 'HiveForge' }])
  ...
  it('loads keyless, keeps the catalog browsable, and fails the request actionably', async () => {
    ...
    expect(ctx.llm.listProviders()).toEqual([{ id: 'hiveforge-official', name: 'HiveForge' }])
    await expect(ctx.llm.listModels('hiveforge-official')).resolves.toHaveLength(3)
    ...
    expect(second.finish.failure.message)
      .toMatch(/store HIVEFORGE_API_KEY through the credentials service.*export HIVEFORGE_API_KEY/s)
  })
  ...
  it('reads the ambient variable when no credentials seam is mounted', async () => {
    ...
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmHiveForge, { baseURL: server.url })
    await assemble(ctx, { model: 'hiveforge-v4-flash', messages: [] })
  ...
  it('prefers explicit config over env for key and base URL', async () => {
    vi.stubEnv('HIVEFORGE_API_KEY', 'env-key')
    vi.stubEnv('HIVEFORGE_BASE_URL', 'http://env-host:1')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
  ...
  it('uses HIVEFORGE_BASE_URL when config omits baseURL', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    vi.stubEnv('HIVEFORGE_BASE_URL', server.url)
    vi.stubEnv('HIVEFORGE_API_KEY', 'test-key')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmHiveForge, {})
  ...
  it('takes HIVEFORGE_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.dsh/.env', values: { HIVEFORGE_BASE_URL: 'https://user.example' } },
    ])
    ...
    const project = createLaunchEnvironmentSnapshot([
      { source: 'project-env', path: '/work/.env', values: { HIVEFORGE_BASE_URL: 'https://project.example' } },
    ])
    ...
    const shell = createLaunchEnvironmentSnapshot([
      { source: 'process', values: { HIVEFORGE_BASE_URL: 'https://stale.example' } },
    ])
  ...
  it('defaults to the public base URL without config or env', async () => {
    vi.stubEnv('HIVEFORGE_API_KEY', 'k')
    vi.stubEnv('HIVEFORGE_BASE_URL', undefined)
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    // Registration succeeds; no call is made (would hit api.hiveforge.com).
    await ctx.plugin(LlmHiveForge, {})
    expect(ctx.llm.listProviders()).toEqual([{ id: 'hiveforge-official', name: 'HiveForge' }])
  })
  ...
  it('adapter is constructible directly for embedding over the shared resolver', async () => {
    const adapter = adapterOf()
    expect(adapter).toBeInstanceOf(HiveForgeAdapter)
    ...
    await expect(adapter.listModels('hiveforge-official')).resolves.toHaveLength(3)
  })
  ...
  it('resolves connection facts and the credential exactly once per stream call', async () => {
    ...
    const adapter = new HiveForgeAdapter({ options, resolveApiKey, resolveUserId })
