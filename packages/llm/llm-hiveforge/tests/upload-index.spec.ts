...
describe('HiveForgeUploadIndex', () => {
  it('normalizes trailing endpoint slashes in the credential scope', () => {
    expect(hiveForgeFileScope('https://api.hiveforge.com///', 'key'))
      .toBe(hiveForgeFileScope('https://api.hiveforge.com', 'key'))
  })
  ...
  it('isolates API-key namespaces and reuses only records above the refresh margin', async () => {
    ...
    const first = hiveForgeFileScope('https://api.hiveforge.com', 'first-key')
    const second = hiveForgeFileScope('https://api.hiveforge.com', 'second-key')
    const record = {
      scope: first,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: HiveForgeFileId('file-api-one'),
      bytes: 3,
      createdAt: 1_000,
      expiresAt: 10_000,
    }
  ...
  it('keeps a reusable cross-process winner and removes only an exact generation', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsh-upload-index-'))
    const index = new HiveForgeUploadIndex(join(dir, 'index.json'))
    const scope = hiveForgeFileScope('https://api.hiveforge.com', 'key')
    const first = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: HiveForgeFileId('file-api-first'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
    const duplicate = { ...first, fileId: HiveForgeFileId('file-api-duplicate') }
  ...
  it('treats a corrupt upload cache as empty and repairs it on the next commit', async () => {
    ...
    const index = new HiveForgeUploadIndex(path)
    const scope = hiveForgeFileScope('https://api.hiveforge.com', 'key')
    const record = {
      scope,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: HiveForgeFileId('file-api-repaired'),
      bytes: 3,
      createdAt: 1,
      expiresAt: 10_000,
    }
  ...
  it.each([
    ...
    await expect(index.get(
      hiveForgeFileScope('https://api.hiveforge.com', 'key'), VARIANT, 1, 1,
  ...
  it('rejects duplicate persisted mappings as a corrupt cache', async () => {
    ...
    const record = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: HiveForgeFileId('file-api-one'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
  ...
  it('drops expired records on commit and clears only the selected namespace', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsh-upload-index-'))
    const index = new HiveForgeUploadIndex(join(dir, 'index.json'))
    const first = hiveForgeFileScope('https://api.hiveforge.com', 'first')
    const second = hiveForgeFileScope('https://api.hiveforge.com', 'second')
    const expired = {
      scope: first, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: HiveForgeFileId('file-api-expired'), bytes: 3, createdAt: 1, expiresAt: 2,
    }
    const live = {
      ...expired, scope: second, fileId: HiveForgeFileId('file-api-live'), expiresAt: 10_000,
    }
