...
import { createUserMessage, CallId, createMessage } from '@hiveforge-ai/dsh-llm'
import type { ContentBlock, Message, TokenUsage } from '@hiveforge-ai/dsh-llm'
import SessionStore, { Session, SessionId, canonicalHeader } from '@hiveforge-ai/dsh-session'
import type { EpochHeader, SessionEvent } from '@hiveforge-ai/dsh-session'
...
import type { TokenMeasurement, TokenMeterConfig } from '@hiveforge-ai/dsh-token-meter'
...
describe('TokenMeter pricing', () => {
  ...
  it('prices header, tools, and surface when no reusable usage exists', () => {
    ...
    appendHeader(session, header('hiveforge-v4-flash', {
      system: 'system',
...
describe('replay anchors and surface folds', () => {
  ...
  it('uses disjoint provider usage and signed durable-output rewrites', () => {
    ...
    appendSuccessfulCall(session, header('hiveforge-v4-flash'), {
      providerText: 'short',
  ...
  it('selects a heuristic anchor when provider usage would undercut its scale', () => {
    ...
    const requestHeader = header('hiveforge-v4-flash', { system })
  ...
  it('uses an estimated anchor when provider usage is absent', () => {
    ...
    appendSuccessfulCall(session, header('hiveforge-v4-flash', { system: 's' }), {
  ...
  it('distinguishes an explicit empty source-event list from an absent legacy list', () => {
    ...
    appendSuccessfulCall(explicit, header('hiveforge-v4-flash'), {
      durableText: 'listener injected text',
      providerText: '',
      usage: USAGE,
      provenance: 'empty',
    })
    appendSuccessfulCall(legacy, header('hiveforge-v4-flash'), {
      durableText: 'listener injected text',
      providerText: '',
      usage: USAGE,
      provenance: 'absent',
  ...
  it('invalidates usage for any canonical envelope change or explicit override', () => {
    ...
    const anchoredHeader = header('hiveforge-v4-flash', { system: 'one' })
    ...
    expect(service.measure(session, header('hiveforge-v4-flash', { system: 'two' })).baseline.kind)
      .toBe('estimated')
    expect(service.measure(session, header('hiveforge-v4-pro', { system: 'one' })).baseline.kind)
      .toBe('estimated')
  ...
  it('folds the latest full header snapshot into the effective envelope', () => {
    ...
    session.append('request/header', {
      header: header('hiveforge-v4-pro'),
      reason: 'change',
    })
  ...
  it('replays seeded append and replace operations with signed deltas', () => {
    ...
    appendSuccessfulCall(original, header('hiveforge-v4-flash'), {
      usage: USAGE,
...
describe('malformed replay and listener lifecycle', () => {
  ...
  it('rejects an assistant without its step boundary transactionally', () => {
    ...
    session.append('assistant/message', {
      ...
      message: createMessage({
        ...
        source: {
          kind: 'model',
          ...{ provider: 'mock', model: 'hiveforge-v4-flash' },
        },
      }),
  ...
  it('clears completed step boundaries and rejects overlapping or late step events', () => {
    ...
    appendHeader(late, header('hiveforge-v4-flash'))
  ...
  it('rejects repeated and non-earlier assistant source-event references', () => {
    ...
    appendHeader(duplicate, header('hiveforge-v4-flash'))
    ...
    appendHeader(future, header('hiveforge-v4-flash'))
