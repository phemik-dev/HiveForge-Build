...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@hiveforge-ai/dsh-token-meter', TokenMeter],
    ['@hiveforge-ai/dsh-compaction-tool-result-pruner', ToolResultPruner],
    ['@hiveforge-ai/dsh-compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@hiveforge-ai/dsh-token-meter'",
      "- name: '@hiveforge-ai/dsh-compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
