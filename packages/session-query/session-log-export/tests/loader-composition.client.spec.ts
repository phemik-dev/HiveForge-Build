...
import * as SessionLogDownload from '@hiveforge-ai/dsh-session-log-export'
...
describe('session-log-download real Loader composition', () => {
  it('discovers and executes /export through the assembled command plane', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@hiveforge-ai/dsh-commands'",
      "- name: '@hiveforge-ai/dsh-session-log-export'",
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@hiveforge-ai/dsh-commands', CommandRuntime],
      ['@hiveforge-ai/dsh-session-log-export', SessionLogDownload],
    ])
