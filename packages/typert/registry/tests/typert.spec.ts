...
} from '@hiveforge-ai/dsh-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@hiveforge-ai/dsh-typert-protocol'
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@hiveforge-ai/dsh-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@hiveforge-ai/dsh-tools', 'ToolInput')).toBe('@hiveforge-ai/dsh-tools#ToolInput')
    expect(typertPackageKey('@hiveforge-ai/dsh-tools', 'host')).toBe('@hiveforge-ai/dsh-tools#host')
    expect(ctx.typert.get('@hiveforge-ai/dsh-tools#ToolInput')).toMatchObject({
      package: '@hiveforge-ai/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@hiveforge-ai/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@hiveforge-ai/dsh-tools', 'host')).toMatchObject({
      key: '@hiveforge-ai/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@hiveforge-ai/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@hiveforge-ai/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@hiveforge-ai/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@hiveforge-ai/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@hiveforge-ai/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@hiveforge-ai/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@hiveforge-ai/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@hiveforge-ai/dsh-tools#ToolInput')).not.toBe(projected)
  })
