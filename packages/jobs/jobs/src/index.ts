 ...
 * process-local registry lives in `@hiveforge-ai/dsh-jobs-local`.
 * @module @hiveforge-ai/dsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@hiveforge-ai/dsh-jobs is the abstract job registry seam; load an implementation such as @hiveforge-ai/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
