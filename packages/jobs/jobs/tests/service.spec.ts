...
import { JobId, JobRegistry } from '@hiveforge-ai/dsh-jobs'
import type {
 ...
 * behavior suite lives with `@hiveforge-ai/dsh-jobs-local`.
 */
...
describe('JobRegistry seam', () => {
  ...
  it('mounting the abstract seam directly fails loudly at load (stale-composition fence)', async () => {
    ...
    await expect(ctx.plugin(JobRegistry as unknown as typeof StubJobRegistry))
      .rejects.toThrow(/abstract job registry seam; load an implementation such as @hiveforge-ai\/dsh-jobs-local/)
  })
})
