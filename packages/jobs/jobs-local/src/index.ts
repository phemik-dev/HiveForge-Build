 ...
 * @module @hiveforge-ai/dsh-jobs-local
 */

import { Context } from '@hiveforge-ai/cordis'
import z from '@hiveforge-ai/schemastery'
import type { Agent } from '@hiveforge-ai/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@hiveforge-ai/dsh-scope'
import type { ScopeLayer } from '@hiveforge-ai/dsh-scope'
import { deadline, timeoutOf } from '@hiveforge-ai/dsh-timeout'
import { JobRegistry, JobId } from '@hiveforge-ai/dsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@hiveforge-ai/dsh-jobs'
 ...
 * `@hiveforge-ai/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @hiveforge-ai/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @hiveforge-ai/dsh-agent)')
    }
