...
import AgentRegistry, { agentEvents } from '@hiveforge-ai/dsh-agent'
import type { Agent } from '@hiveforge-ai/dsh-agent'
import AttachmentStore from '@hiveforge-ai/dsh-attachment'
import LlmRuntime, { LlmAdapter, ReasoningEffortId } from '@hiveforge-ai/dsh-llm'
import type {
...
async function harness(logged?: {
  ...
  ctx.llm.registerAdapter(['hiveforge-official'], new CatalogAdapter('HiveForge', [
    { provider: 'hiveforge-official', id: 'hiveforge-chat', name: 'HiveForge Chat' },
    { provider: 'hiveforge-official', id: 'hiveforge-reasoner', name: 'HiveForge Reasoner', description: 'Reasoning model' },
  ], REASONING))
...
describe('Web session model selection', () => {
  it('validates an ordered image batch before persisting any member', async () => {
    ...
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'hiveforge-official', model: 'hiveforge-chat' }),
      cwd: '/tmp',
    })
  ...
  it('groups successful providers and leaves an unlisted current selection out of the catalog', async () => {
    ...
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'hiveforge-official', model: 'hiveforge-chat' }), cwd: '/tmp' })
    ...
    expect(catalog.groups).toEqual([{
      ...
      name: 'HiveForge',
      models: [
        { id: 'hiveforge-chat', name: 'HiveForge Chat', reasoning: REASONING },
        {
          id: 'hiveforge-reasoner',
          name: 'HiveForge Reasoner',
  ...
  it('accepts an advisory-unlisted model, rejects an unavailable provider, and switches only after the next assembly', async () => {
    ...
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'hiveforge-official', model: 'hiveforge-chat' })
    expect((await ctx.systemPrompt.assemble()).variables)
      .toMatchObject({ provider: 'hiveforge-official', model: 'hiveforge-chat' })
    ...
    )).resolves.toMatchObject({ provider: 'hiveforge-official', model: 'hiveforge-chat' })

    expect((await ctx.systemPrompt.assemble()).variables)
      .toMatchObject({ provider: 'hiveforge-official', model: 'private-preview' })
    await expect(agentEvents(ctx, agent).waterfall(
    ...
    expect(unsupported.result).toMatchObject({
      ...
      error: {
        ...
        message: 'provider "hiveforge-official" model "private-preview" does not support reasoning effort "medium"',
      },
    })
    ...
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'hiveforge-official', model: 'private-preview', reasoningEffort: 'max' })
    await ctx.fiber.dispose()
  })

  it('reads the Agent default live for a session whose log names no selection', async () => {
    const { ctx, sessionId } = await harness()
    let stored = { provider: 'hiveforge-official', model: 'hiveforge-chat' }
    const api = createApiProxy(ctx, {
    ...
    stored = { provider: 'hiveforge-official', model: 'hiveforge-reasoner' }
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'hiveforge-official', model: 'hiveforge-reasoner' })
    expect(expectValue(await api.host.describe(request({}))))
      .toMatchObject({ provider: 'hiveforge-official', model: 'hiveforge-reasoner' })
    await ctx.fiber.dispose()
  })
  ...
  it('saves an accepted selection as the default and survives a storage failure', async () => {
    ...
    expectValue(await api.sessions.selectModel(request({
      sessionId, provider: 'hiveforge-official', model: 'hiveforge-reasoner', reasoningEffort: 'max',
    })))
    expect(saved).toEqual([
      { provider: 'hiveforge-official', model: 'hiveforge-reasoner', reasoningEffort: 'max' },
    ])
    ...
    const stillAccepted = expectValue(await api.sessions.selectModel(request({
      sessionId, provider: 'hiveforge-official', model: 'hiveforge-chat',
    })))
    expect(stillAccepted.selected).toEqual({ provider: 'hiveforge-official', model: 'hiveforge-chat', reasoningEffort: 'high' })
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'hiveforge-official', model: 'hiveforge-chat', reasoningEffort: 'high' })
    await ctx.fiber.dispose()
  })
  ...
  it('refuses a prompt no adapter can route, and reports it on the directory', async () => {
    ...
    expectValue(await api.sessions.selectModel(request({
      sessionId, provider: 'hiveforge-official', model: 'unlisted-but-served',
    })))
    const catalog = expectValue(await api.sessions.models(request({ sessionId })))
