...
import { installModelSelection } from '@hiveforge-ai/dsh-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@hiveforge-ai/dsh-agent'
import type {} from '@hiveforge-ai/dsh-agent-presets/types'
import { AttachmentError, admitEncodedImages } from '@hiveforge-ai/dsh-attachment'
import type { ImageAttachmentRef } from '@hiveforge-ai/dsh-attachment'
import { createUserMessage, freezeMessage, ReasoningEffortId } from '@hiveforge-ai/dsh-llm'
import { errorChain } from '@hiveforge-ai/dsh-llm'
import type { ContentBlock, MessageSource } from '@hiveforge-ai/dsh-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@hiveforge-ai/dsh-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@hiveforge-ai/dsh-session'
import type { SessionPersistence } from '@hiveforge-ai/dsh-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@hiveforge-ai/dsh-session-query'
import { SubagentError } from '@hiveforge-ai/dsh-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@hiveforge-ai/dsh-subagent'
import { isUserInvocable } from '@hiveforge-ai/dsh-skill'
import type { Workspace, WorkspaceRecord } from '@hiveforge-ai/dsh-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@hiveforge-ai/dsh-workspace'
// Type-only: brings the `ctx.tools` Context merge into this program (viewFor reads presenters).
import {
  InvalidPresetIdError, PresetExistsError, PresetMountError,
  PresetNotWritableError, resolveSessionPreset, UnknownPresetError,
} from '@hiveforge-ai/dsh-agent-presets'
import type { PresetBearingSession } from '@hiveforge-ai/dsh-agent-presets'
...
import type {} from '@hiveforge-ai/dsh-jobs'
import type { JobSnapshot } from '@hiveforge-ai/dsh-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@hiveforge-ai/dsh-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@hiveforge-ai/dsh-goal'
import type { GoalRef as CoreGoalRef } from '@hiveforge-ai/dsh-goal'
...
import type {} from '@hiveforge-ai/dsh-cordis-host-runner/types'
import type {} from '@hiveforge-ai/dsh-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@hiveforge-ai/dsh-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@hiveforge-ai/dsh-settings'
import { credentialRef, parseCredentialKey } from '@hiveforge-ai/dsh-credentials'
import { parseCredentialsDocument } from '@hiveforge-ai/dsh-credentials-local'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@hiveforge-ai/dsh-session-title'
import type { CallId } from '@hiveforge-ai/dsh-llm/brand'
import type { ScopeKey } from '@hiveforge-ai/dsh-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@hiveforge-ai/dsh-user-approval'
...
import { UserQuestionError } from '@hiveforge-ai/dsh-user-questions'
import { DirectoryPickerError } from '@hiveforge-ai/dsh-host-directory-picker'
import {
...
} from '@hiveforge-ai/dsh-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @hiveforge-ai/dsh-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @hiveforge-ai/dsh-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @hiveforge-ai/dsh-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @hiveforge-ai/dsh-credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @hiveforge-ai/dsh-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @hiveforge-ai/dsh-skill', details: {} })
        }
    ...
    migration: {
      ...
      async apply(request) {
        ...
        const result: MigrationApplyValue = {
          legacyHome,
          settings: {
            imported: [],
            skipped: [],
