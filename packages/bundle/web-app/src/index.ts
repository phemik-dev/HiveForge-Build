/**
 * @hiveforge-ai/dsh-web-app — the browser-surface bundle's runtime glue plugin
 ...
 * patch.
 * @module @hiveforge-ai/dsh-web-app
 */
...
import { addHarnessSourceSection } from '@hiveforge-ai/dsh-app-boot'
import * as FrontendStatic from '@hiveforge-ai/dsh-host-frontend-static'
...
function webSurfacePrompt(webUrl: string): string {
  ...
  return `You are interacting with the user through the HiveForge desktop app at ${webUrl}. `
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@hiveforge-ai/dsh-web-frontend/dist/index.html')
  } catch {
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [DSH_WEB_URL]: { description: 'Canonical local URL of the HiveForge app serving this session.' },
        },
  ...
  if (config.printUrl || handoffBrowser) {
    ...
    const announceReady = (): void => {
      ...
      if (config.printUrl) {
        console.log(`HiveForge: ${webUrl}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
      }
      if (handoffBrowser) {
        console.log('HiveForge: opening the default browser; pass --no-open to disable')
