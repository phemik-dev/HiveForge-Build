...
import * as Spawn from '@hiveforge-ai/dsh-subagent-spawn-in-process'
import WorkerThreadWorkflowEngine from '../src/index.ts'

/**
 * With-key e2e: a REAL script in a REAL worker thread
 * drives REAL spawn children against the live HiveForge API — one plain child
...
async function harness(): Promise<Context> {
  ...
  await built.plugin(LlmHiveForge)
...
describe.skipIf(!process.env.HIVEFORGE_API_KEY)('worker workflow engine with-key e2e', () => {
