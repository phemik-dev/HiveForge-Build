 ...
 * multi-step tool turn (plus a follow-up turn) against the live HiveForge API must report
...
async function loopHarness(): Promise<Context> {
  ...
  await created.plugin(LlmHiveForge)
...
describe.skipIf(!process.env.HIVEFORGE_API_KEY)('log-derived request cache hits (real API)', () => {
  it('every request after the first hits the provider prefix cache', async () => {
    ctx = await loopHarness()
    const agent = ctx.agentLoop.create(SessionId('cache-e2e'), { provider: 'hiveforge-official', model: 'hiveforge-v4-flash' })
