 ...
 *
 * @module @hiveforge-ai/dsh-system-prompt
 */

import { Context, Service } from '@hiveforge-ai/cordis'
import z from '@hiveforge-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@hiveforge-ai/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@hiveforge-ai/dsh-scope'
import type { ContextSnapshotSection, ToolSchema } from '@hiveforge-ai/dsh-llm'
...
declare module '@hiveforge-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@hiveforge-ai/dsh-scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by HiveForge Harness.',
      })
    }
    this.section({
