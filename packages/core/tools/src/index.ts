 ...
 * execution pipeline.
 * @module @hiveforge-ai/dsh-tools
 */

import { Context, Service } from '@hiveforge-ai/cordis'
import z from '@hiveforge-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@hiveforge-ai/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@hiveforge-ai/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@hiveforge-ai/dsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@hiveforge-ai/dsh-llm'
import type { Agent } from '@hiveforge-ai/dsh-agent'
import { snapshotJsonValue } from '@hiveforge-ai/dsh-session'
import type { JsonValue, UserMessage } from '@hiveforge-ai/dsh-session'
import type { ToolProviderResult } from '@hiveforge-ai/dsh-system-prompt'
import type { CodeRuntime } from '@hiveforge-ai/dsh-code-runtime'
...
export type { JsonValue } from '@hiveforge-ai/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@hiveforge-ai/dsh-tools`
...
declare module '@hiveforge-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@hiveforge-ai/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@hiveforge-ai/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@hiveforge-ai/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@hiveforge-ai/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@hiveforge-ai/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @hiveforge-ai/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
