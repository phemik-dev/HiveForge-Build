...
import { createUserMessage, CallId  } from '@hiveforge-ai/dsh-llm'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@hiveforge-ai/dsh-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@hiveforge-ai/dsh-tools'
import type { Agent } from '@hiveforge-ai/dsh-agent'
import { Session, SessionId } from '@hiveforge-ai/dsh-session'
import type { JsonValue, SessionEventMap } from '@hiveforge-ai/dsh-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@hiveforge-ai/dsh-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
