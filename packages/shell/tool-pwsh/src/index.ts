 ...
 * `@hiveforge-ai/dsh-pwsh-local`) backs `ctx.shell`; the tool contract is
 ...
 * exit-status parse from `@hiveforge-ai/dsh-shell`.
 *
 * @module @hiveforge-ai/dsh-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@hiveforge-ai/cordis'
import z from '@hiveforge-ai/schemastery'
import { defineTool, TOOL_ABORTED } from '@hiveforge-ai/dsh-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@hiveforge-ai/dsh-tools'
...
import type { SandboxExecutionPolicy, SandboxMode } from '@hiveforge-ai/dsh-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@hiveforge-ai/dsh-sandbox'
import type { SandboxPolicyService } from '@hiveforge-ai/dsh-sandbox-policy'
import type { ShellRunResult } from '@hiveforge-ai/dsh-shell'
import { parseExitStatus } from '@hiveforge-ai/dsh-shell'
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: PwshToolArgs, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @hiveforge-ai/dsh-jobs and @hiveforge-ai/dsh-tool-jobs')
        }
