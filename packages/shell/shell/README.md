...
| Package | Role |
|---|---|
| `@hiveforge-ai/dsh-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@hiveforge-ai/dsh-bash-local` | Service Provider: local subprocesses |
| `@hiveforge-ai/dsh-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@hiveforge-ai/dsh-tool-bash` | the model-facing tool schemas over `ctx.shell` |
