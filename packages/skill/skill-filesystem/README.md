...
This package implements one skill source. It scans local project, custom, and user skill roots, parses `SKILL.md` or flat Markdown skill files, and registers the provider on `ctx.skills`. The registry remains in `@hiveforge-ai/dsh-skill`; the durable session catalogs and model-facing loader tool remain in `@hiveforge-ai/dsh-tool-skill`.
...
| `dshHome` | `$DSH_HOME` or `~/.dsh` | HiveForge Harness config root resolved by [`@hiveforge-ai/dsh-home-paths`](../../util/home-paths/README.md); scans `skills` under this directory. |
