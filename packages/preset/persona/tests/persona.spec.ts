...
import { createScope, type ScopeKey } from '@hiveforge-ai/dsh-scope'
import { describe, expect, it } from 'vitest'
import * as Persona from '@hiveforge-ai/dsh-persona'
import { PERSONA_SECTION } from '@hiveforge-ai/dsh-persona'
...
describe('the persona row', () => {
  ...
  it('interpolates prompt variables strictly, like any other section', async () => {
    const ctx = await harness('')
    const key: ScopeKey = { agent: 'a1' }
    ctx.systemPrompt.variable('model', () => 'hiveforge-v4-pro')
    ...
    expect(renderPrompt(await ctx.systemPrompt.assemble({ scope: key })))
      .toContain('You run on hiveforge-v4-pro.')
  })
