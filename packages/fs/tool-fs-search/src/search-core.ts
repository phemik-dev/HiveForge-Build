 ...
 *
 * @module @hiveforge-ai/dsh-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@hiveforge-ai/dsh-output-retention'
import type { RetainedItems } from '@hiveforge-ai/dsh-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@hiveforge-ai/dsh-subprocess'
 ...
 * `@hiveforge-ai/dsh-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@hiveforge-ai/dsh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@hiveforge-ai/dsh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@hiveforge-ai/dsh-tool-fs-search/presentation} `grepSearchMeta`)
