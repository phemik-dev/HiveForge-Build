...
import * as GoalSessionInvariant from '@hiveforge-ai/dsh-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@hiveforge-ai/dsh-goal-round-driver'
import InvariantRegistry, { InvariantError } from '@hiveforge-ai/dsh-invariants'
import SessionStore, { SessionId, type Session } from '@hiveforge-ai/dsh-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@hiveforge-ai/dsh-goal-round-driver',
    }))
  })
