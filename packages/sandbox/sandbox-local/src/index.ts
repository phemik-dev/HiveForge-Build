 ...
 * @module @hiveforge-ai/dsh-sandbox-local
 */
...
} from '@hiveforge-ai/node-addon-landlock-run'
import { Context } from '@hiveforge-ai/cordis'
import z from '@hiveforge-ai/schemastery'
import { assertNever } from '@hiveforge-ai/dsh-llm'
import { SandboxProvider, SandboxUnavailableError } from '@hiveforge-ai/dsh-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@hiveforge-ai/dsh-sandbox'
import type { SessionId } from '@hiveforge-ai/dsh-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@hiveforge-ai/dsh-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@hiveforge-ai/dsh-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@hiveforge-ai/dsh-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@hiveforge-ai/dsh-sandbox-windows-acl/src/runner.ts'))
