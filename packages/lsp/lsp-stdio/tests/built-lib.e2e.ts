...
/**
 * Keyless built-artifact smoke: plain Node imports `@hiveforge-ai/dsh-lsp` and
 * `@hiveforge-ai/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@hiveforge-ai/cordis')
      const { default: Lsp } = await import('@hiveforge-ai/dsh-lsp')
      const LspLocal = await import('@hiveforge-ai/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@hiveforge-ai/dsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@hiveforge-ai/dsh-subprocess-local')
