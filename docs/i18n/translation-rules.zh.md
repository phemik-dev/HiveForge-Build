...
本文规定本仓库文档配对的翻译方式。英文是规范维护版本（见 [README.md](README.zh.md)）：无后缀的英文文件是默认参考与维护源，中文是可选的经评审对侧。本文规则约束如何更新中文对侧，以及修复英文源文时少数必要的反向翻译。这些规则对人类和 agent（智能体）同等生效。日常工作中，agent 会在术语指导下直接一次完成有改动内容的翻译；扩展版 [.agents/skills/dsh-translate-docs](../../.agents/skills/dsh-translate-docs/SKILL.md) 工作流仅在用户显式调用时运行。规则级别沿用 RFC 2119 的用法：**必须（MUST）** ／ **禁止（MUST NOT）** 会卡门禁或评审；**应当（SHOULD）** 偏离时要说明理由；**可以（MAY）** 自行裁量。

## 忠实性

- 中文对侧*必须*传达与规范英文侧相同的内容：不添加行为、前置条件、警告、版本声明或示例，也不漏掉任何一项。如果两侧在实质内容上不一致，请在必要时先修正英文源文，并在同一个变更里同步更新中文对侧。
...
- 专有名词保持规范大小写：GitHub、TypeScript、HiveForge。除非引用代码，否则绝不写 `github`／`Github`。
