...
The public package family belongs to the `@hiveforge-ai` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@hiveforge-ai/node-addon-landlock-run
@hiveforge-ai/node-addon-landlock-run-<platform>
```
