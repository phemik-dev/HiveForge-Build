 ...
 * @module @hiveforge-ai/dsh/profile-boot
 */
...
import { DSH_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@hiveforge-ai/dsh-launch-environment'
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@hiveforge-ai/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@hiveforge-ai/cordis-plugin-hmr', config: { root: [] } })
      }
