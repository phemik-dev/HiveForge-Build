...
import { boot, loadOverlayPatches } from '@hiveforge-ai/dsh-app-boot'
import SystemPrompt from '@hiveforge-ai/dsh-system-prompt'
import ToolRuntime from '@hiveforge-ai/dsh-tools'
import * as McpClient from '@hiveforge-ai/dsh-mcp-client/src/index.ts'
...
describe('third-party memory MCP example overlays', () => {
  it.each(examples)('parses $file with the documented generic plugin fields', (contract) => {
    ...
    expect(row.name).toBe('@hiveforge-ai/dsh-mcp-client')
    ...
    expect(source).not.toContain('HIVEFORGE_API_KEY')
  })
