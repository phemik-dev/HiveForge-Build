...
import { entryListSchema } from '@hiveforge-ai/cordis-plugin-include'
import { evaluate } from '@hiveforge-ai/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@hiveforge-ai/dsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@hiveforge-ai/dsh-base', '@hiveforge-ai/dsh-web-app'])
    ...
    for (const name of ['@hiveforge-ai/dsh-pwsh-sandbox', '@hiveforge-ai/dsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@hiveforge-ai/dsh-base'])
