...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@hiveforge-ai/dsh-base`, `@hiveforge-ai/dsh-web-app`, `@hiveforge-ai/dsh-headless`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.
