...
## Do not import `@hiveforge-ai/dsh-client-*` here
...
`@hiveforge-ai/dsh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@hiveforge-ai/dsh-client-web` and the boot-manifest
type from `@hiveforge-ai/dsh-client-modules/client`: booting the real shell is what
...
`@hiveforge-ai/dsh-client-runtime/client` because `client/runtime` is reachable
