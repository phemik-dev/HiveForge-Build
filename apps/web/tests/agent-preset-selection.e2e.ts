...
describe('web e2e: agent-preset selection', () => {
  ...
  it('names every preset and what it is for', async () => {
    ...
    expect(snapshot).toContain('Essentials')
  ...
  it('labels a resumed session with the preset it was created under', async () => {
    ...
    expect(snapshot).toContain('Essentials')
    expect(snapshot).toContain('button "1 supporting agent"')
    expect(snapshot.indexOf('button "1 supporting agent"')).toBeLessThan(snapshot.indexOf('Essentials'))
    expect(snapshot.indexOf('Essentials')).toBeLessThan(snapshot.indexOf('button "Session log"'))
    // Static chrome, not a control: the header can only report a composition
    // the host would refuse to change.
    expect(snapshot).not.toContain('button "Essentials"')
  })
