...
import type {} from '@hiveforge-ai/dsh-session-reference/types'
...
describe.skipIf(MODE === 'record')('web e2e: file and session references through the real host', () => {
  ...
  it('renders the durable direct-message then recall order', async () => {
    ...
    await target.click()
    await page.getByRole('button', { name: /^Recalled context\s*Research notes$/ }).waitFor({ timeout: 15_000 })

    const snapshot = (await captureStableAria(page, '[class*="centerCol"]', scaffold.workspaceCwd))
      .split(TARGET_SESSION_ID).join('{{targetId}}')
    await compareOrRefreshGolden(ORDER_EXPECTED, snapshot, MODE)
    expect(snapshot.indexOf('Research notes what changed?')).toBeLessThan(snapshot.indexOf('Recalled context Research notes'))
    expect(tripwire.pageErrors).toEqual([])
    expect(tripwire.warnings).toEqual([])
