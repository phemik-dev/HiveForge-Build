...
import { bootInjections, orderByModuleGraph } from '@hiveforge-ai/dsh-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry } from '@hiveforge-ai/dsh-client-modules/client'
import { AppWebEntry } from '@hiveforge-ai/dsh-client-web'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@hiveforge-ai/dsh-app-boot')).href) as unknown as BootComposition
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  for (const id of ['@hiveforge-ai/dsh-client-modules', '@hiveforge-ai/dsh-client-runtime']) {
