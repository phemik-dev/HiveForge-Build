...
// drives `web_search`; the model stream is replayed while the real HiveForge
...
import { WEB_SEARCH_MAX_RESULTS } from '@hiveforge-ai/dsh-tool-web'
import {
...
const QUERIES = ['HiveForge Harness snapshot search', 'HiveForge Harness multi-query search'] as const
...
/** Start the deterministic HiveForge Messages double used by the real provider. */
...
describe('web e2e: shipped default web search', () => {
  ...
  it.skipIf(MODE === 'record')('uses the real provider and persists the capped structured result', () => {
    ...
    const auxiliaryRequests = sessionEvents.filter(
      (event): event is Extract<SessionEvent, { type: 'web/hiveforge-search-llm-request' }> =>
        event.type === 'web/hiveforge-search-llm-request',
    )
