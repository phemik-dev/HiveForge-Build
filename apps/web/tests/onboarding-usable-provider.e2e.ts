...
// for the official HiveForge key again, and the first-run setup card is a card
// they can close. The shipped HiveForge adapter stays mounted without a
...
describe.skipIf(MODE === 'record')('web e2e: another usable provider ends first-run onboarding', () => {
  ...
  beforeAll(async () => {
    scaffold = await launchWebScaffold({ hiveForgeMissingCredential: true, seedLocalePreference: 'zh' })
  ...
  it('closes the setup card without discarding the add card beside it', async () => {
    ...
    await settings.getByRole('button', { name: '编辑 HiveForge (hiveforge-official)' }).waitFor({ timeout: 10_000 })
  ...
  it('stops prompting for HiveForge once the other provider can serve requests', async () => {
    ...
    // Only minimax-cn is reachable; HiveForge still holds no credential.
    ...
    expect(credentials).not.toContain('HIVEFORGE_API_KEY')
    ...
    // The Models page agrees: HiveForge stays a row rather than reopening its
