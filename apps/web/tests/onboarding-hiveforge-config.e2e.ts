// Keyless browser e2e: the shipped HiveForge adapter stays mounted while its
...
const SNAPSHOT_DIR = fileURLToPath(new URL('./snapshots/onboarding-hiveforge-config', import.meta.url))
...
describe.skipIf(MODE === 'record')('web e2e: first-run HiveForge credential setup', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-hiveforge-config'))
    ...
    expect(stored.includes(`HIVEFORGE_API_KEY: ${secret}`)).toBe(true)
    ...
    const hiveForgeRow = settings.getByText('HiveForge', { exact: true }).first()
    await hiveForgeRow.waitFor({ timeout: 10_000 })
    await hiveForgeRow.locator('xpath=ancestor::li').getByRole('button', { name: '编辑' }).click()
  ...
  it('configures arbitrary HiveForge models and prompts after the selected model is removed', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-hiveforge-models'))
    ...
    await settings.waitFor({ timeout: 10_000 })
    await settings.getByRole('button', { name: '模型' }).click()
    const hiveForge = settings.getByText('HiveForge', { exact: true }).first()
    await hiveForge.waitFor({ timeout: 10_000 })
    await hiveForge.locator('xpath=ancestor::li').getByRole('button', { name: '编辑' }).click()
    ...
    expect(document).toContain('id: hiveforge-v4-pro')
    expect(document).toContain('id: hiveforge-v4-flash-vision-exp')
    ...
    expect(document).not.toMatch(/^\s*- id: hiveforge-v4-flash$/m)
    ...
    expect(await page.getByText('hiveforge-v4-flash', { exact: true }).count()).toBe(0)
    await page.getByRole('menuitemradio', { name: 'HiveForge-V4-Flash-Vision-Exp' }).waitFor({ timeout: 10_000 })
