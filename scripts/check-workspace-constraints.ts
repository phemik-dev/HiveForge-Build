...
const vendoredPackages = new Set([
  '@hiveforge-ai/cordis',
  '@hiveforge-ai/cosmokit',
  '@hiveforge-ai/schemastery',
  '@hiveforge-ai/cordis-plugin-loader',
  '@hiveforge-ai/cordis-plugin-include',
  '@hiveforge-ai/cordis-plugin-group',
  '@hiveforge-ai/cordis-plugin-timer',
  '@hiveforge-ai/cordis-plugin-hmr',
  '@hiveforge-ai/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@hiveforge-ai/node-addon-landlock-run',
  '@hiveforge-ai/node-addon-landlock-run-linux-arm64',
  '@hiveforge-ai/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@hiveforge-ai/node-addon-landlock-run': ['src/main.c'],
}
const repositoryUrl = 'git+https://github.com/hiveforge-harness/hiveforge-harness.git'
/**
 ...
 */
const publishedRepositoryUrl = 'git+https://github.com/hiveforge-ai/hiveforge-harness.git'
...
const experimentalPackageNamePrefix = '@hiveforge-ai/dsh-experimental-'
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/

const localArtifactDirs = new Set(['node_modules'])
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@hiveforge-ai/dsh': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@hiveforge-ai/dsh-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@hiveforge-ai/dsh-client-ui-primitives': ['lib/**/*.css'],
  '@hiveforge-ai/dsh-client-web': ['lib/**/*.css'],
  '@hiveforge-ai/dsh-client-ui-theme': ['lib/styles'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@hiveforge-ai/dsh-code-runtime-python': ['py/**/*.py'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@hiveforge-ai/dsh-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@hiveforge-ai/dsh-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  // SQLite loads every statement from immutable package resources at runtime.
  '@hiveforge-ai/dsh-session-persistence-sqlite': ['resources/sql/**/*.sql'],
  '@hiveforge-ai/dsh-skill-badge': ['assets'],
  '@hiveforge-ai/dsh-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (manifest.name?.startsWith('@hiveforge-ai/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@hiveforge-ai/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@hiveforge-ai/dsh-')) {
    const peer = manifest.peerDependencies?.['@hiveforge-ai/cordis']
