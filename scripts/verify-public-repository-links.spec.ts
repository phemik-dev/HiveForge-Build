...
describe('repository link policy', () => {
  it('rejects encoded and case-varied references to the unavailable repository', () => {
    const unavailableOwner = ['hiveforge', 'ai'].join('-')
    const unavailableName = ['hiveforge', 'harness', 'sdk'].join('-')
    ...
    const source = [
      'https://github.com/hiveforge-ai/hiveforge-harness',
  ...
  it('preserves frozen archived Agent Notes', () => {
    const unavailableRepository = ['hiveforge-ai', 'hiveforge-harness-sdk'].join('/')
