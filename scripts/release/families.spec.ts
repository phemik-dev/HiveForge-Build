...
describe('release families', () => {
  it('excludes private experimental packages from the dsh release', () => {
    ...
    expect(members.map(member => member.name)).not.toContain('@hiveforge-ai/dsh-experimental-agent-team')
  })
  ...
  it('bumps private dsh packages without adding release tags', () => {
    ...
    const published = member('packages/core/published', '@hiveforge-ai/dsh-published')
  ...
  it('names one tag for the whole dsh family and one per vendored package', () => {
    ...
    const cli = member('apps/cli', '@hiveforge-ai/dsh')
    const cordis = { ...member('vendor/cordis', '@hiveforge-ai/cordis'), version: '4.0.1' }
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@hiveforge-ai/dsh'), { ...member('apps/web', '@hiveforge-ai/dsh-web-frontend'), version: '0.0.2' }]
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@hiveforge-ai/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@hiveforge-ai/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@hiveforge-ai/dsh-consumer', { dependencies: { '@hiveforge-ai/dsh-library': 'workspace:^' } }),
      member('packages/a/library', '@hiveforge-ai/dsh-library'),
      member('packages/a/zebra', '@hiveforge-ai/dsh-zebra'),
    ]

    expect(dsh.publishOrder(members).order.map(entry => entry.name)).toEqual([
      '@hiveforge-ai/dsh-library',
      '@hiveforge-ai/dsh-consumer',
      '@hiveforge-ai/dsh-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@hiveforge-ai/dsh-left', { dependencies: { '@hiveforge-ai/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@hiveforge-ai/dsh-right', { dependencies: { '@hiveforge-ai/dsh-left': 'workspace:^' } }),
    ]
  ...
  it('publishes a peer before its consumer', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@hiveforge-ai/dsh-consumer', { peerDependencies: { '@hiveforge-ai/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@hiveforge-ai/dsh-zebra'),
    ]
  ...
  it('orders around a peer cycle rather than refusing to publish, and reports the edge it dropped', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@hiveforge-ai/dsh-left', { peerDependencies: { '@hiveforge-ai/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@hiveforge-ai/dsh-right', { peerDependencies: { '@hiveforge-ai/dsh-left': 'workspace:^' } }),
    ]
  ...
  it('honours an install edge even when a peer cycle surrounds it', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/base', '@hiveforge-ai/dsh-base', { peerDependencies: { '@hiveforge-ai/dsh-consumer': 'workspace:^' } }),
      member('packages/a/consumer', '@hiveforge-ai/dsh-consumer', {
        dependencies: { '@hiveforge-ai/dsh-base': 'workspace:^' },
        peerDependencies: { '@hiveforge-ai/dsh-base': 'workspace:^' },
      }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@hiveforge-ai/dsh-base', peer: '@hiveforge-ai/dsh-consumer' },
    ])
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@hiveforge-ai/dsh-alpha', { peerDependencies: { '@hiveforge-ai/dsh-bravo': 'workspace:^' } }),
      member('packages/a/bravo', '@hiveforge-ai/dsh-bravo', { peerDependencies: { '@hiveforge-ai/dsh-charlie': 'workspace:^' } }),
      member('packages/a/charlie', '@hiveforge-ai/dsh-charlie', { dependencies: { '@hiveforge-ai/dsh-alpha': 'workspace:^' } }),
    ]
    ...
    expect(() => { dsh.publishOrder(members) }).toThrow(/no publish order honours @hiveforge-ai\/dsh-charlie -> @hiveforge-ai\/dsh-alpha/)
  })

  it('ignores devDependencies when ordering', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@hiveforge-ai/dsh-alpha', { devDependencies: { '@hiveforge-ai/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@hiveforge-ai/dsh-zebra'),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@hiveforge-ai/dsh-library')
    const vendored = member('vendor/cordis', '@hiveforge-ai/cordis')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@hiveforge-ai/dsh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@hiveforge-ai/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@hiveforge-ai/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@hiveforge-ai/dsh-library', { files: ['lib/index.js'] }),
