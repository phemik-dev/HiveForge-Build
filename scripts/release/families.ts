...
const WORKSPACE_ROOT_PACKAGE = '@hiveforge-ai/dsh-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@hiveforge-ai/')) throw new Error(`${normalized} must name an @hiveforge-ai package`)
...
class DshFamily extends ReleaseFamily {
  ...
  readonly installedEntry = { packageName: '@hiveforge-ai/dsh', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@hiveforge-ai/', '')}-v`
  }
