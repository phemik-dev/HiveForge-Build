...
export function renderPairMeta(source: string, sourceHash: string, zh: string, zhHash: string): string {
  return [
    ...
    '# side as of the last confirmed-consistent state. English is the canonical maintained copy;',
    '# keep the reviewed Chinese counterpart aligned and re-record with:',
...
export const TRANSLATION_SCOPE_GLOB_EXCLUDES = [
  ...
  'apps/web/dist/**',
  '.artifacts/**',
  'python/sdk-runtime/src/hiveforge_harness_runtime/runtime/dsh-jsonrpc-agent-*/**',
  'python/sdk-runtime/src/hiveforge_harness_runtime/runtime/node/**',
  'vendor/**',
]
...
function isTranslationSourceExcluded(file: string): boolean {
  ...
  return segments.some(segment => NON_SOURCE_DIRECTORIES.has(segment)
    ...
    || file.startsWith('python/sdk-runtime/src/hiveforge_harness_runtime/runtime/dsh-jsonrpc-agent-')
    || file.startsWith('python/sdk-runtime/src/hiveforge_harness_runtime/runtime/node/')
}
...
const PUBLIC_REPOSITORY_BLOB_ROOT = 'https://github.com/hiveforge-ai/hiveforge-harness/blob/master/'
