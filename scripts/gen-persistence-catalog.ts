...
const SESSION_PACKAGE = '@hiveforge-ai/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@hiveforge-ai/dsh-session/types'
 ...
 * top-level declaration (in `@hiveforge-ai/dsh-session`) and any declaration
 * merge inside a `declare module '@hiveforge-ai/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @hiveforge-ai/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
