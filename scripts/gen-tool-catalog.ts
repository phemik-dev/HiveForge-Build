...
import GoalService from '@hiveforge-ai/dsh-goal'
...
import { PwshLocalExecutor } from '@hiveforge-ai/dsh-pwsh-local'
...
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@hiveforge-ai/dsh-attachment'
...
import type { SubagentProvider, SubagentReportDelivery } from '@hiveforge-ai/dsh-subagent'
import * as ToolSubagentControl from '@hiveforge-ai/dsh-tool-subagent-control'
import * as ToolSubagentListAgents from '@hiveforge-ai/dsh-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@hiveforge-ai/dsh-tool-subagent-report'
import SkillRegistry from '@hiveforge-ai/dsh-skill'
import * as SkillFileSystem from '@hiveforge-ai/dsh-skill-filesystem'
import LocalJobRegistry from '@hiveforge-ai/dsh-jobs-local'
import * as ToolAskUser from '@hiveforge-ai/dsh-tool-ask-user'
import * as ToolBash from '@hiveforge-ai/dsh-tool-bash'
import * as ToolPwsh from '@hiveforge-ai/dsh-tool-pwsh'
import * as ToolBashPersistent from '@hiveforge-ai/dsh-tool-bash-persistent'
...
import * as ToolStrReplaceEditor from '@hiveforge-ai/dsh-tool-str-replace-editor'
import TerminalSessionService from '@hiveforge-ai/dsh-terminal'
import * as ToolPty from '@hiveforge-ai/dsh-tool-terminal'
import * as ToolGoal from '@hiveforge-ai/dsh-tool-goal'
import * as ToolSchedule from '@hiveforge-ai/dsh-schedule'
import Lsp from '@hiveforge-ai/dsh-lsp'
import * as ToolLsp from '@hiveforge-ai/dsh-tool-lsp'
import * as ToolSkill from '@hiveforge-ai/dsh-tool-skill'
import * as ToolSessionQuery from '@hiveforge-ai/dsh-tool-session-query'
import * as ToolTasks from '@hiveforge-ai/dsh-tool-jobs'
import type TeamService from '@hiveforge-ai/dsh-experimental-agent-team'
import * as ToolTeam from '@hiveforge-ai/dsh-experimental-tool-agent-team'
import * as ToolTodo from '@hiveforge-ai/dsh-tool-todo'
import * as ToolSubagent from '@hiveforge-ai/dsh-tool-subagent'
import * as ToolWeb from '@hiveforge-ai/dsh-tool-web'
import VmWorkflowEngine from '@hiveforge-ai/dsh-workflow-worker-thread'
import * as ToolRalph from '@hiveforge-ai/dsh-tool-ralph'
import * as ToolWorkflow from '@hiveforge-ai/dsh-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@hiveforge-ai/dsh-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  {
    pkg: '@hiveforge-ai/dsh-plan-mode',
    dir: 'plan-mode',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-bash',
    dir: 'tool-bash',
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@hiveforge-ai/dsh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@hiveforge-ai/dsh-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@hiveforge-ai/dsh-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@hiveforge-ai/dsh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@hiveforge-ai/dsh-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@hiveforge-ai/dsh-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@hiveforge-ai/dsh-tool-bash-persistent',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-pwsh-persistent',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@hiveforge-ai/dsh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@hiveforge-ai/dsh-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-terminal',
    dir: 'tool-terminal',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-goal',
    dir: 'tool-goal',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@hiveforge-ai/dsh-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@hiveforge-ai/dsh-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-skill',
    dir: 'tool-skill',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-session-query',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-subagent',
    dir: 'tool-subagent',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-subagent-control',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-subagent-report',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-jobs',
    dir: 'tool-jobs',
  ...
  {
    pkg: '@hiveforge-ai/dsh-experimental-tool-agent-team',
    dir: 'tool-agent-team',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-todo',
    dir: 'tool-todo',
  ...
  {
    pkg: '@hiveforge-ai/dsh-tool-workflow',
    dir: 'tool-workflow',
