...
const RELEASE_ENTRY_PACKAGE = '@hiveforge-ai/dsh'
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@hiveforge-ai/')) {
        throw new Error(`${manifestPath} must name an @hiveforge-ai package`)
      }
      if (name === '@hiveforge-ai/dsh-root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@hiveforge-ai/dsh/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@hiveforge-ai/') || name === '@hiveforge-ai/dsh-root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.HIVEFORGE_API_KEY = 'keyless-installed-web-no-call'
